...
async function getTorExitIPAsync() {
  ...
  try {
    ...
    console.error('ERROR : Firewall : Unable to refresh Tor exit IP list from check.torproject.org')
  }
  ...
  if (extractedTorExitIPs !== null) {
    ...
    try {
      ...
      console.error('ERROR : Firewall : Unable to save Tor exit IP list to cache')
    }
    ...
    try {
      ...
      console.error('ERROR : Firewall : Unable to load Tor exit IP list from cache')
      return []
    }
  }
}
...
async function getLocalIPBlacklistAsync() {
  try {
    ...
    console.log('WARN : Firewall : Unable to parse local IP blacklist (ip-blacklist.txt) ')
    return []
  }
}
...
function setupCommonRestifyConfigAndRoutes(server) {
  ...
  server.get({ path: '/calendar/:tx_id/data', version: '2.0.0' }, apiCalendar.getDataValueByIDAsync)
  ...
  server.get({ path: '/reputation/:min/:max', version: '2.0.0' }, apiReputation.getReputationDataRangeByIdsAsync)
  // get the reputation item for the specified id
  server.get({ path: '/reputation/:id', version: '2.0.0' }, apiReputation.getReputationDataByIdAsync)
  // get the most recent reputation items
  server.get({ path: '/reputation/recent', version: '2.0.0' }, apiReputation.getRecentReputationDataAsync)
  ...
  return new Promise((resolve, reject) => {
    restifyServer.listen(HTTP_PORT, err => {
      ...
      if (process.env.NODE_ENV !== 'test')
        console.log(`INFO : App : ${restifyServer.name} listening HTTP on port ${HTTP_PORT}`)
...
async function startSecureRestifyServerAsync() {
  ...
  return new Promise((resolve, reject) => {
    restifyServer.listen(HTTPS_PORT, err => {
      if (err) return reject(err)
      if (process.env.NODE_ENV !== 'test')
        console.log(`INFO : App : ${restifyServer.name} listening HTTPS on port ${HTTPS_PORT}`)
      return resolve(restifyServer)
    })
  })
}
