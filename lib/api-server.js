...
async function initRestifyServerAsync (mergedIPList) {
  return new Promise(async (resolve, reject) => {
    try {
      if (server) {
        server.close(async () => {
          await restifySetupAsync(mergedIPList)
          return resolve()
        })
      } else {
        await restifySetupAsync(mergedIPList)
...
async function restifySetupAsync (mergedIPList) {
  // RESTIFY SETUP
...
async function startAsync () {
  ...
  try {
    ...
    await initRestifyServerAsync(mergedIPList)
  } catch (error) {
