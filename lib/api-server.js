...
const apiReputation = require('./endpoints/reputation.js')
...
function setupCommonRestifyConfigAndRoutes(server) {
  ...
  // submit hash(es)
  server.post({ path: '/hashes', version: '2.0.0' }, throttle(50, 25), ensureAcceptingHashes, apiHashes.postHashesAsync)
  // get a data value from a calendar transaction
  server.get({ path: '/calendar/:tx_id/data', version: '2.0.0' }, throttle(15, 5), apiCalendar.getDataValueByIDAsync)
  // get a single proof with a single hash_id
  server.get({ path: '/proofs/:hash_id_node', version: '2.0.0' }, throttle(15, 5), apiProofs.getProofsByIDAsync)
  // get multiple proofs with 'hashids' header param
  server.get({ path: '/proofs', version: '2.0.0' }, throttle(15, 5), apiProofs.getProofsByIDAsync)
  // verify one or more proofs
  server.post({ path: '/verify', version: '2.0.0' }, throttle(15, 5), apiVerify.postProofsForVerificationAsync)
  // get a range of reputation items by ids, inclusive
  server.get(
    { path: '/reputation/:min/:max', version: '2.0.0' },
    throttle(1, 0.5),
    apiReputation.getReputationDataRangeByIdsAsync
  )
  // get the reputation item for the specified id
  server.get({ path: '/reputation/:id', version: '2.0.0' }, throttle(1, 0.5), apiReputation.getReputationDataByIdAsync)
  // get the most recent reputation items
  server.get(
    { path: '/reputation/recent', version: '2.0.0' },
    throttle(1, 0.5),
    apiReputation.getRecentReputationDataAsync
  )
  // get configuration information for this Node
  server.get({ path: '/config', version: '2.0.0' }, throttle(1, 1), apiConfig.getConfigInfoAsync)
...
async function startInsecureRestifyServerAsync() {
  ...
  return new Promise((resolve, reject) => {
    restifyServer.listen(HTTP_PORT, err => {
      ...
      logger.info(`App : Chainpoint Node listening HTTP on port ${HTTP_PORT}`)
...
async function startSecureRestifyServerAsync() {
  ...
  return new Promise((resolve, reject) => {
    restifyServer.listen(HTTPS_PORT, err => {
      if (err) return reject(err)
      logger.info(`App : Chainpoint Node listening HTTPS on port ${HTTPS_PORT}`)
      return resolve(restifyServer)
    })
  })
}
