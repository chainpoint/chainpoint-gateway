...
// Restify server objects to be initialized
...
function setupCommonRestifyConfigAndRoutes(server) {
  ...
  // submit hash(es)
  server.post({ path: '/hashes', version: '1.0.0' }, ensureAcceptingHashes, apiHashes.postHashesAsync)
  // get a single proof with a single hash_id
  server.get({ path: '/proofs/:hash_id_node', version: '1.0.0' }, apiProofs.getProofsByIDAsync)
  // get multiple proofs with 'hashids' header param
  server.get({ path: '/proofs', version: '1.0.0' }, apiProofs.getProofsByIDAsync)
  // verify one or more proofs
  server.post({ path: '/verify', version: '1.0.0' }, apiVerify.postProofsForVerificationAsync)
  // get configuration information for this Node
  server.get({ path: '/config', version: '1.0.0' }, apiConfig.getConfigInfoAsync)

  server.get({ path: '/stats', version: '1.0.0' }, apiStats.getNodeStatsAsync)
...
async function startAsync() {
  try {
    IPBlacklist = await initIPBlacklistAsync()
    insecureServer = await startInsecureRestifyServerAsync()
    secureServer = await startSecureRestifyServerAsync()
  } catch (error) {
