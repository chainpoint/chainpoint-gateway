...
async function getTorExitIPAsync() {
  ...
  if (extractedTorExitIPs !== null) {
    try {
      await rocksDB.setAsync(TOR_IPS_KEY, extractedTorExitIPs)
      return extractedTorExitIPs
    } catch (error) {
    ...
    try {
      ...
      let compactTorExitIPs = _.compact(cachedTorExitIPs)
