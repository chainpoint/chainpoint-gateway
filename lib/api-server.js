...
async function initCoreIPBlacklistAsync () {
  try {
    ...
    let filteredBlacklist = _.filter(response.blacklist, (ip) => { return validator.isIP(ip.toString()) })
...
function ipFilter (req, res, next) {
  ...
  reqIPs = reqIPs.filter((ip) => validator.isIP(ip.toString())).reduce((ips, ip) => {
...
async function startRestifyServerAsync () {
  // RESTIFY SETUP
