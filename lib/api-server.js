...
function initRestifyServer (mergedIPList) {
  // RESTIFY SETUP
...
async function getServerAsync () {
  // sometime the server is requested before it is initialized
  // prevent this from happening by retrying in case it is still null
  let restApi = server
  while (restApi === null) {
...
module.exports = {
  ...
  setPublicKeySet: (pubKeys) => { apiVerify.setPublicKeySet(pubKeys) },
