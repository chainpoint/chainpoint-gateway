...
async function startInsecureRestifyServerAsync() {
  ...
  return new Promise((resolve, reject) => {
    restifyServer.listen(HTTP_PORT, err => {
      ...
      if (process.env.NODE_ENV !== 'test') logger.info(`App : Chainpoint Node listening HTTP on port ${HTTP_PORT}`)
...
async function startSecureRestifyServerAsync() {
  ...
  return new Promise((resolve, reject) => {
    restifyServer.listen(HTTPS_PORT, err => {
      if (err) return reject(err)
      if (process.env.NODE_ENV !== 'test') logger.info(`App : Chainpoint Node listening HTTPS on port ${HTTPS_PORT}`)
      return resolve(restifyServer)
    })
  })
}
