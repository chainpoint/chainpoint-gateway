...
function getKeyNamesForTypeAndTimestamp(type, timestamp) {
  ...
  return [yearStatsKey, monthStatsKey, dateStatsKey, hourStatsKey].map(
    key => `${key}:${type}`
  )
}
...
async function logRecentHash(hashIdNode, hash, submittedAt) {
  ...
  if (RECENT_HASH_DATA.length > RECENT_HASH_COUNT)
    RECENT_HASH_DATA.length = RECENT_HASH_COUNT
}

function getMetricsByTypeAndTimestamp(type, timestamp) {
  let keys = getKeyNamesForTypeAndTimestamp(type, timestamp)
  let last24HrsSum = getLast24HourKeysForType(type)
...
function pruneHoulyMetricDataOlderThan(timestamp) {
  ...
  for (let key in COUNT_METRICS) {
    let matchInfo = key.match(
      /^nodestats:counter:(\d{4}:\d{2}:\d{2}:\d{2}):.+$/
    )
...
module.exports = {
  captureEvent: (type, value) => {
    captureEvent(type, value)
  },
  logRecentHash: (hashIdNode, hash, submittedAt) => {
    logRecentHash(hashIdNode, hash, submittedAt)
  },
