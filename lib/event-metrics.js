...
function pruneHourlyMetricDataOlderThan(timestamp) {
  let prunedKeys = []
...
async function persistDataAsync() {
  ...
  let prunedKeys = pruneHourlyMetricDataOlderThan(
    moment
      .utc(Date.now())
      .add(-3, 'd')
      .valueOf()
  ...
  try {
    ...
    logger.error('Could not save updated count metrics')
  }
  // Save latest RECENT_HASH_DATA values
  try {
    await rocksDB.saveRecentHashDataAsync(getRecentHashDataObjectArray())
  } catch (error) {
    logger.error('Could not save recent hash data metrics')
  }
}
