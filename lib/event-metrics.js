...
function getKeyNamesForTypeAndTimestamp (type, timestamp) {
  ...
  let yearStatsKey = `nodestats:counter:${type}:${utc.format('YYYY')}`
...
function getLast24HourKeysForType (type) {
  ...
  let keys = []
  for (let x = 0; x < 24; x++) {
    keys.push(`nodestats:counter:${type}:${utc.format('YYYY:MM:DD:HH')}`)
    utc.add(-1, 'h')
  }
  return keys
}
...
async function persistDataAsync () {
  ...
  try {
    await rocksDB.saveCountMetricsAsync(getMetricsObjectArray())
  } catch (error) {
