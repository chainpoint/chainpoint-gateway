...
async function generateReputationEntryAsync() {
  // If this is in Private Network Mode, skip Reputation generation, as it does not apply
  ...
  try {
    ...
    logger.info(`Reputation : Generating new item with id ${newRepItem.id}`)
    ...
    logger.info(`Reputation : Successfully added new item with id ${newRepItem.id}`)
  } catch (error) {
    logger.error(`Could not create reputation item : ${error}`)
  }
}
...
async function retrieveAndStoreProofAsync(repId, hashIdNode) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    logger.info(`Reputation : Retrying proof retrieval for id ${hashIdNode}`)
  }
