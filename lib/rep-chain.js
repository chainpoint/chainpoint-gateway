...
async function generateReputationEntryAsync() {
  try {
    ...
    console.log(`INFO : Reputation : Generating new item with height ${newRepItem.id}`)

    // submit latest cal block hash to this Node and receive
    // the CAL proof's hash id for includion in the new reputation item
    // and the hint as to when to retrive the CAL proof
    let submitResult = await submitHashToSelfAsync(newRepItem.calBlockHash)
    ...
    console.log(`INFO : Reputation : Successfully added new item with height ${newRepItem.id}`)
  } catch (error) {
...
async function retrieveAndStoreProofAsync(repHeight, hashIdNode) {
  let options = {
    Accept: 'application/vnd.chainpoint.ld+json',
    method: 'GET',
    uri: `http://127.0.0.1/proofs/${hashIdNode}`,
    gzip: true,
    json: true,
    timeout: 1000
  }
  ...
  try {
    // save this new proof
    await rocksDB.saveReputationItemProofAsync(repHeight, proof)
  } catch (error) {
    throw `Unable to store proof for hashIdNode ${hashIdNode} at height ${repHeight}`
  }
}
