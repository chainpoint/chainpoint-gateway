  ...
  if (hashesForTree.length > 0) {
    ...
    try {
      ...
      submitResults = submitResults.filter(submitResult => {
        ...
        logger.info(
          `Aggregator : Core IP : ${submitResult.coreIP} : proofId : ${submitResult.proofId} : ${JSON.stringify(submitResult)} : ${hashesForTree.length}`
        )
        // validate BLAKE2s
        ...
        let expectedData = Buffer.concat([Buffer.from([0x01]), h.digest().slice(54)]).toString('hex')
        let embeddedData = submitResult.proofId.slice(24)
        if (embeddedData !== expectedData) {
          logger.error(`Aggregator : Submit : HashID from Core refused : Cannot validate embedded BLAKE2s data : ${embeddedData} != ${expectedData}`)
