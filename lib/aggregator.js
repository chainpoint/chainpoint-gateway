  ...
  if (hashesForTree.length > 0) {
    ...
    try {
      ...
      if (embeddedData !== expectedData) {
        throw new Error('Aggregator : Hash ID from Core refused: Cannot validate embedded BLAKE2s data')
      }
      ...
      try {
        ...
        throw new Error(`Aggregator : failed saving proof for key : ${newKey}`)
      }
      ...
      try {
        ...
        throw new Error(`Aggregator : failed write to hash_id table for key : ${newKey}`)
      }

      console.log(`INFO : Aggregator : ${hashesForTree.length} : ${nodeAggregationData.hash_id_core} : ${submitResult.tnt_credit_balance}`)
    } catch (error) {
      console.error(`ERROR : Aggregator : failed hash submit : ${error.message}`)
    }
  }
}
