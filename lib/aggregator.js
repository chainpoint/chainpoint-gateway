...
async function aggregateAndSendToCoreAsync () {
  ...
  if (hashesForTree.length > 0) {
    ...
    for (let x = 0; x < treeSize; x++) {
      ...
      nodeProofDataItem.proofState = formatAsChainpointV3Ops(partialProofPath, 'sha-256')
    ...
    try {
      ...
      try {
        await db.saveProofStateBatchAsync(nodeProofDataItems)
      } catch (error) {
        let err = `Unable to persist proof state data to disk : ${error.message}`
        throw err
      }

      console.log(`INFO : Aggregator : ${hashesForTree.length} : ${submitResult.hash_id} : ${submitResult.tnt_credit_balance}`)
    } catch (error) {
