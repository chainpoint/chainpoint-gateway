...
async function aggregateAndSendToCoreAsync () {
  ...
  if (hashesForTree.length > 0) {
    ...
    try {
      let submitResult = await submitHashToCoreAsync([coreAggregationData.merkle_root])
      ...
      if (error.message === 'Bad UUID time') {
...
async function checkForProofsFromCoreAsync () {
  ...
  // TODO: Just use websockets and forget about the previous issues
...
async function submitHashToCoreAsync (hashArray) {
  let options = {
    headers: {
  ...
  try {
    ...
    if (error.statusCode === 500 && error.message === 'Bad UUID time') throw new Error('Bad UUID time')
  ...
  let result = {
    processing_hints: response.meta.processing_hints,
    hash_id: response.hashes[0].hash_id
  }
  return result
}
