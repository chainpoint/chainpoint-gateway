...
async function aggregateAndSendToCoreAsync () {
  ...
  // flatten the ip array into a CSV string
  let ipCSVList = ipsForTree.join(',')
  ...
  if (hashesForTree.length > 0) {
    ...
    try {
      ...
      let submitResult = await submitHashToCoreAsync(nodeAggregationData.merkle_root, ipCSVList, hmac)
      ...
      try {
        ...
        throw new Error(`Could not save proof to Redis for ${newKey}: ${error.message}`)
      }
      ...
      try {
        ...
        throw new Error(`Could not write to hash_id lookup table: ${error.message}`)
      }
...
async function submitHashToCoreAsync (hash, ipCSVList, hmac) {
  let options = {
    headers: {
  ...
  try {
    ...
    if (error.statusCode) throw new Error(`ERROR : Invalid response : ${error.statusCode} : ${error.message}`)
    throw new Error(`ERROR : No response received on POST hashes : ${error.message}`)
  }
