...
module.exports = {
  startAggInterval: (newInterval) => {
    ...
    if (_.isUndefined(aggForCoreIntervalInstance)) {
      AGG_FOR_CORE_INTERVAL_MS = newInterval
      aggForCoreIntervalInstance = setInterval(aggregateAndSendToCoreAsync, newInterval)
    } else {
      if (newInterval && newInterval !== AGG_FOR_CORE_INTERVAL_MS) {
