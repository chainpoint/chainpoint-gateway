...
async function getActiveUsageTokenAsync() {
  ...
  try {
    ...
    if (isExpired && balance >= 1) {
      ...
      // update aud and aulr based on new active token, for evaluation later
      decodedToken = jwt.decode(activeUsageToken, { complete: true })
      aud = decodedToken.payload.aud
      aulr = decodedToken.payload.aulr
    }
    // if the token audience does not reference the same IPs as this Node's current connected Core IP list, update the token audience
    let connectedCoreIPs = cores.getCoreConnectedIPs()
    let audIPs = aud.split(',')
    let ipSetsDifferent = false
    for (let audIP of audIPs) {
      if (!connectedCoreIPs.includes(audIP)) {
        ipSetsDifferent = true
        break
      }
    }
    if (ipSetsDifferent) {
