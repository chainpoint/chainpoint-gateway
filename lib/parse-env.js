...
function valSocket(value) {
  try {
    ...
    let host = segments[0]
    let port = segments[1]
    if (!validator.isFQDN(host) && !validator.isIP(host) && host !== 'localhost') throw new Error()
    if (!validator.isPort(port)) throw new Error()
    return value
  } catch (error) {
    throw new Error('The supplied value must be a valid <host>:<port> string')
  }
}
...
let envDefinitions = {
  ...
  NETWORK: envalid.str({ default: 'testnet', desc: `The network to use, 'mainnet' or 'testnet'` }),
