...
async function captureEvent (redis, eventType, incrementValue) {
  try {
    // nodestats:${moment.utc().year()}:${moment.utc().month()}
    await redis.incrbyAsync(`nodestats:${eventType}:${moment.utc().year()}`, incrementValue)
    await redis.incrbyAsync(`nodestats:${eventType}:${moment.utc().year()}:${moment.utc().month()}`, incrementValue)
    await redis.incrbyAsync(`nodestats:${eventType}:${moment.utc().year()}:${moment.utc().month()}:${moment.utc().date()}`, incrementValue)
    ...
    if (_.isNull((await redis.getAsync(`nodestats:${eventType}:${moment.utc().year()}:${moment.utc().month()}:${moment.utc().date()}:${moment.utc().hour()}`)))) {
      await redis.setAsync(`nodestats:${eventType}:${moment.utc().year()}:${moment.utc().month()}:${moment.utc().date()}:${moment.utc().hour()}`, 0, 'EX', 86400 * 3)
    }
    await redis.incrbyAsync(`nodestats:${eventType}:${moment.utc().year()}:${moment.utc().month()}:${moment.utc().date()}:${moment.utc().hour()}`, incrementValue)
  } catch (e) {
...
module.exports = {
  ...
  captureEvent: _.curry(captureEvent)
}
