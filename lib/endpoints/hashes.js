...
function generatePostHashesResponseMetadata() {
  ...
  if (env.CHAINPOINT_NODE_REFLECTED_URI && env.CHAINPOINT_NODE_REFLECTED_URI !== 'empty') {
    metaDataObj.uri = env.CHAINPOINT_NODE_REFLECTED_URI === 'public' ? env.CHAINPOINT_NODE_PUBLIC_URI : env.CHAINPOINT_NODE_PRIVATE_URI
  }
...
function generateProcessingHints(timestampDate) {
  ...
  let oneHourFromTopOfTheHour = new Date(
    twoHoursFromTimestamp.setHours(twoHoursFromTimestamp.getHours(), 0, 0, 0)
  )
  let calHint = utils.formatDateISO8601NoMs(
    utils.addSeconds(timestampDate, 10 + aggregator.getAggIntervalSeconds())
  )
...
async function postHashesV1Async(req, res, next) {
  ...
  if (!req.params.hasOwnProperty('hashes')) {
    return next(
      new errors.InvalidArgumentError('invalid JSON body, missing hashes')
    )
  }
  ...
  if (!_.isArray(req.params.hashes)) {
    return next(
      new errors.InvalidArgumentError(
        'invalid JSON body, hashes is not an Array'
      )
    )
  }
  ...
  if (_.size(req.params.hashes) < 1) {
    return next(
      new errors.InvalidArgumentError(
        'invalid JSON body, hashes Array is empty'
      )
    )
  }
  ...
  if (_.size(req.params.hashes) > env.POST_HASHES_MAX) {
    return next(
      new errors.InvalidArgumentError(
        `invalid JSON body, hashes Array max size of ${
          env.POST_HASHES_MAX
        } exceeded`
      )
    )
  ...
  if (!containsValidHashes) {
    return next(
      new errors.InvalidArgumentError(
        'invalid JSON body, invalid hashes present'
      )
    )
  }

  let responseObj = generatePostHashesResponse(req.params.hashes)
  // Increase hashes event metric by responseObj.hashes.length
  eventMetrics.captureEvent('hashes', responseObj.hashes.length)
  // Add new hashes to recent hashes array
  for (let hashObj of responseObj.hashes.slice(
    Math.max(responseObj.hashes - 25, 0)
  )) {
    eventMetrics.logRecentHash(
      hashObj.hash_id_node,
      hashObj.hash,
      responseObj.meta.submitted_at
    )
  }
...
module.exports = {
  ...
  setHmacKey: hmacKey => {
    aggregator.setHmacKey(hmacKey)
  },
  startAggInterval: interval => {
    aggregator.startAggInterval(interval)
  }
}
