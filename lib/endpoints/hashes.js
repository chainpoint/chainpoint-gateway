...
function generatePostHashesResponseMetadata () {
  ...
  metaDataObj.processing_hints = {
    cal: utils.formatDateISO8601NoMs(utils.addSeconds(timestamp, 15)),
...
async function postHashesV1Async (req, res, next) {
  ...
  try {
    const numOfHashes = responseObj.hashes.length
    // nodestats:${moment.utc().year()}:${moment.utc().month()}
    await redis.incrbyAsync(`nodestats:${moment.utc().year()}`, numOfHashes)
    await redis.incrbyAsync(`nodestats:${moment.utc().year()}:${moment.utc().month()}`, numOfHashes)
    await redis.incrbyAsync(`nodestats:${moment.utc().year()}:${moment.utc().month()}:${moment.utc().date()}`, numOfHashes)
    // Set TTL to nodeStats:<year>:<month>:<day>:<hour> to (86400 * 3)
    if (_.isNull((await redis.getAsync(`nodestats:${moment.utc().year()}:${moment.utc().month()}:${moment.utc().date()}:${moment.utc().hour()}`)))) {
      await redis.setAsync(`nodestats:${moment.utc().year()}:${moment.utc().month()}:${moment.utc().date()}:${moment.utc().hour()}`, 0, 'EX', 86400 * 3)
    }
    await redis.incrbyAsync(`nodestats:${moment.utc().year()}:${moment.utc().month()}:${moment.utc().date()}:${moment.utc().hour()}`, numOfHashes)
    // Persist hash_id_node to a capped redis list for the Node Stats UI -> nodestats:last25HashIds
    responseObj.hashes.forEach((currVal) => {
      redis.lpush('nodestats:hashes:last:25', `${currVal.hash_id_node}|${responseObj.meta.submitted_at}`)
      redis.ltrim('nodestats:hashes:last:25', 0, 25)
    })
  } catch (e) {
    return next(new errors.InternalServerError('Could not save statistical hash data'))
  }

  let sourceIp = req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress

  // store hash data for later aggregation
  try {
    await aggregator.addHashDataAsync(responseObj.hashes, sourceIp)
  } catch (error) {
...
module.exports = {
  ...
  startAggInterval: () => { aggregator.startAggInterval() }
}
