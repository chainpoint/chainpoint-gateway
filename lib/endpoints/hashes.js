...
function generatePostHashesResponseMetadata() {
  ...
  if (env.CHAINPOINT_NODE_REFLECTED_URI && env.CHAINPOINT_NODE_REFLECTED_URI !== 'empty') {
    metaDataObj.uri =
      env.CHAINPOINT_NODE_REFLECTED_URI === 'public' ? env.CHAINPOINT_NODE_PUBLIC_URI : env.CHAINPOINT_NODE_PRIVATE_URI
  }
 ...
 *
 */
// TODO: This contains an assumed hard-coded calendar aggregation time of 60 seconds, improve?
// Also, adjust hints to accomodate for new Core interval processes
function generateProcessingHints(timestampDate) {
  ...
  let calHint = utils.formatDateISO8601NoMs(utils.addSeconds(timestampDate, 60 + aggregator.getAggIntervalSeconds()))
...
async function postHashesAsync(req, res, next) {
  ...
  for (let hashObj of responseObj.hashes.slice(Math.max(responseObj.hashes - 25, 0))) {
