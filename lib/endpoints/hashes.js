...
async function postHashesV1Async (req, res, next) {
  ...
  EVENTS.push({ eventType: 'hashes', eventValue: responseObj.hashes.length })
  // captureEvent('hashes', responseObj.hashes.length)
  // Persist hash_id_node to a capped redis list for the Node Stats UI -> nodestats:last25HashIds
  for (let currVal of responseObj.hashes) {
    LAST25.push(`${currVal.hash_id_node}|${currVal.hash}|${responseObj.meta.submitted_at}`)
  }
  // responseObj.hashes.forEach((currVal) => {
  //  redis.lpush('nodestats:hashes:last:25', `${currVal.hash_id_node}|${currVal.hash}|${responseObj.meta.submitted_at}`)
  //  redis.ltrim('nodestats:hashes:last:25', 0, 25)
  // })

  // store hash data for later aggregation
  try {
    await aggregator.addHashDataAsync(responseObj.hashes)
  } catch (error) {
...
async function processAccumulatedEvents () {
  let events = EVENTS.splice(0)
  let last25 = LAST25.splice(0).splice(-25)
  ...
  for (let item of last25) {
    multi.lpushAsync('nodestats:hashes:last:25', item)
  }
  ...
  try {
    ...
    return console.error('Could not save statistical hash data')
  }
}

setInterval(processAccumulatedEvents, PROCESS_EVENTS_INTERVAL_SECONDS * 1000)
