    ...
    case 'cal':
      try {
        ...
        if (!block) {
          ...
          block = { hash: response.body }
        }
        return (block.hash === expectedValue)
      } catch (error) {
        throw new Error(`Cannot perform cal anchor confirmation: ${error.message}`)
      }
    case 'btc':
      try {
        ...
        if (!block) {
          ...
          let response = await rp(options)
          block = { dataVal: response.body }
        }
        let blockRoot = block.dataVal.match(/.{2}/g).reverse().join('')
        return (blockRoot === expectedValue)
      } catch (error) {
        throw new Error(`Cannot perform btc anchor confirmation: ${error.message}`)
      }
    case 'eth':
      break
  }
...
async function postProofsForVerificationV1Async (req, res, next) {
  ...
  try {
    ...
    console.error(error)
    return next(new restify.InternalError('Node internal error verifying proof(s)'))
  }
