    ...
    for (let x = 0; x < verifyTask.anchors.length; x++) {
      try {
        ...
        let anchorResult = {
          ...
          valid: confirmResult,
          block_id: anchor.anchor.anchor_id || undefined,
          block_value: anchor.anchor.expected_value || undefined
        }
        totalCount++
...
function buildResultObject(parseObj, proofIndex) {
  ...
  let hashSubmittedNodeAt =
    parseObj !== null ? parseObj.hash_submitted_node_at : undefined
  let hashIdCore = parseObj !== null ? parseObj.hash_id_core : undefined
  let hashSubmittedCoreAt =
    parseObj !== null ? parseObj.hash_submitted_core_at : undefined
  let expectedValues =
    parseObj !== null ? flattenExpectedValues(parseObj.branches) : undefined
    ...
    case 'cal':
      try {
        ...
        if (!block) {
          ...
          let prefixedSigValues = block.sig.split(':')
          ...
          if (!isValidSig) {
            console.error(
              `ERROR : Calendar : Verify : Bad block signature for Calendar block ${
                block.id
              }`
            )
            return false
...
async function postProofsForVerificationV1Async(req, res, next) {
  ...
  if (!req.params.hasOwnProperty('proofs')) {
    return next(
      new errors.InvalidArgumentError('Invalid JSON body, missing proofs')
    )
  }
  ...
  if (!_.isArray(req.params.proofs)) {
    return next(
      new errors.InvalidArgumentError(
        'Invalid JSON body, proofs is not an Array'
      )
    )
  }
  ...
  if (_.size(req.params.proofs) < 1) {
    return next(
      new errors.InvalidArgumentError(
        'Invalid JSON body, proofs Array is empty'
      )
    )
  }
  ...
  if (_.size(req.params.proofs) > env.POST_VERIFY_PROOFS_MAX) {
    return next(
      new errors.InvalidArgumentError(
        `Invalid JSON body, proofs Array max size of ${
          env.POST_VERIFY_PROOFS_MAX
        } exceeded`
      )
    )
  }
  ...
  try {
    ...
    return next(
      new errors.InternalError('Node internal error verifying proof(s)')
    )
  }
