...
async function confirmExpectedValueAsync(anchorInfo) {
  ...
  // check Core for the block
  // TODO: Update references to `block` to `transaction`
  let txInfo = await cores.getCachedTransactionAsync(anchorCalBlockHeight)
  if (txInfo === null || txInfo.dataVal === undefined) {
    throw new Error('Unable to retrieve transaction from Core to the confirm the anchor value')
  }

  switch (anchorInfo.type) {
    case 'cal':
      return txInfo.dataVal === expectedValue
    case 'btc':
      return txInfo.dataVal === expectedValue
    default:
      return false
