...
async function getCalBlockByHeightV1Async (req, res, next) {
  ...
  if (!_.isInteger(height) || height < 0) {
    return next(new restify.InvalidArgumentError('Invalid height, must be a positive integer.'))
  }
  ...
  try {
    ...
    return next(new restify.InternalError('Node internal error retrieving Calendar block by height'))
  }
}
