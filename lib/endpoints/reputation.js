...
async function getReputationDataRangeByIdsAsync(req, res, next) {
  ...
  if (maxId - minId + 1 > GET_REPUTATION_MAX) {
    return next(new errors.InvalidArgumentError(`invalid request, limit of ${GET_REPUTATION_MAX} items per request`))
  }
