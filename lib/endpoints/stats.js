...
async function getNodeStats(req, res, next) {
  ...
  if (
    ...
    res.send(
      401,
      'Unauthorized: Node is not Public. Please provide a valid authentication header value.'
    )
    return next()
  }
  ...
  if (
    nodeUIPasswordBooleanCheck(env.CHAINPOINT_NODE_UI_PASSWORD) !== false &&
    ((env.CHAINPOINT_NODE_UI_PASSWORD &&
      env.CHAINPOINT_NODE_UI_PASSWORD !== req.headers['auth']) ||
      (!env.CHAINPOINT_NODE_UI_PASSWORD &&
        req.headers['auth'] !== env.NODE_TNT_ADDRESS))
  ) {
    res.send(
      401,
      'Unauthorized: Please provide a valid authentication header value.'
    )
    return next()
  }
  ...
  try {
    ...
    if (!req.query.filter || req.query.filter === 'last_1_days') {
      ...
      let stats = eventMetrics.getMetricsByTypeAndTimestamp(
        'hashes',
        Date.now()
      )
    ...
    res.send(200, {
      ...
      nodeData: {
        ...
        dataFromCoreLastReceived:
          (await rocksDB.getAsync('dataFromCoreLastReceived')) || ''
      }
    })
    return next()
  } catch (e) {
