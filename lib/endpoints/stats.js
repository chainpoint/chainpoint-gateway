...
async function getLast24HashesReceived () {
  ...
  let todayKeys = await redis.keysAsync(`nodestats:hashes:${today.year()}:${today.month()}:${today.date()}:*`)
  let yesterdayKeys = await redis.keysAsync(`nodestats:hashes:${yesterday.year()}:${yesterday.month()}:${yesterday.date()}:*`)
...
async function getNodeStats (req, res, next) {
  ...
  if (req.headers['auth'] === '' && nodeUIPasswordBooleanCheck(env.CHAINPOINT_NODE_UI_PASSWORD) !== false) {
  ...
  try {
    ...
    if (!req.query.filter || req.query.filter === 'last_1_days') {
      ...
      let stats = {
        year: parseInt((await redis.getAsync(`nodestats:hashes:${moment.utc().year()}`)) || 0, 10),
        month: parseInt((await redis.getAsync(`nodestats:hashes:${moment.utc().year()}:${moment.utc().month()}`)) || 0, 10),
        day: parseInt((await redis.getAsync(`nodestats:hashes:${moment.utc().year()}:${moment.utc().month()}:${moment.utc().date()}`)) || 0, 10),
        last24Hrs: await getLast24HashesReceived(),
        hour: parseInt((await redis.getAsync(`nodestats:hashes:${moment.utc().year()}:${moment.utc().month()}:${moment.utc().date()}:${moment.utc().hour()}`)) || 0, 10)
      }
    ...
    let nodeData = (await redis.getAsync('dataFromCore')) || {}
