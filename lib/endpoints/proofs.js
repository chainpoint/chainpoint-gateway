...
async function getProofsByIDAsync(req, res, next) {
  ...
  for (let proofId of proofIds) {
    ...
    var startProofEvent = {
      ...
      ea: 'GetProof',
      el: proofId,
      ev: 'Start', //hash might not be present, use "Start" to indicate start of endpoint
      dp: '/proof'
    }
  ...
  for (let nodeProofDataItem of nodeProofDataItems) {
    ...
    var event = {
      ec: 'Gateway',
      el: result.proof_id,
      ev: result.proof ? result.proof.hash : 'null',
      dp: '/proof'
    }
    if (result.anchors_complete.length == 0) {
      event.ea = 'GetProofFail'
      analytics.sendEvent(event)
    } else if (result.anchors_complete.includes('btc') || result.anchors_complete.includes('tbtc')) {
      event.ea = 'GetProofDataBtc'
      analytics.sendEvent(event)
    } else if (result.anchors_complete.includes('cal') || result.anchors_complete.includes('tcal')) {
