...
async function getProofsByIDV1Async (req, res, next) {
  ...
  hashIdCoreBatches.forEach((hashIdCoreBatch) => {
    getProofTasks.push(async () => { return getProofsFromProofProxyAsync(hashIdCoreBatch) })
  })
...
async function getProofsFromProofProxyAsync (hashIds) {
  let options = {
    headers: {
  ...
  for (let x = 0; x < response.length; x++) {
    ...
    if (hashItem.proof === null) {
      ...
      try {
        proofObject = chpBinary.binaryToObjectSync(hashItem.proof)
      } catch (err) {
