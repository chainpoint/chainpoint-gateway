...
async function getProofsByIDV1Async (req, res, next) {
  ...
  let proofResults
  try {
    proofResults = await cachedProofs.getCachedCoreProofsAsync(uniqueHashIdCores)
  } catch (error) {
  ...
  let coreProofResults = proofResults.reduce((results, proofItem) => {
    results[proofItem.hash_id] = proofItem
    return results
  }, {})
