...
async function getProofsByIDV1Async (req, res, next) {
  ...
  for (let x = 0; x < idCoreHostBatches.length; x++) {
    getProofTasks.push(getProofsFromCoreByHashIdsAsync(idCoreHostBatches[x]))
  }
  // await the resolution of all promises and then process the array of results
  let resultBatches = await Promise.all(getProofTasks).catch(() => { console.error(`ERROR : Could not get proofs from Core`) })
  for (let x = 0; x < idCoreHostBatches.length; x++) {
