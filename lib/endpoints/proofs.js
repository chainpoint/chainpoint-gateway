...
async function getProofsByIDV1Async (req, res, next) {
  ...
  for (let x = 0; x < uniqueHashIdCoreValues.length; x++) {
    try {
      ...
      console.error(`Could not get proof for ${uniqueHashIdCoreValues[x]} from Core: ${error.message}`)
    }
  }
  ...
  try {
    ...
    console.error(`Could not retrieve aggregation data from Redis for ${aggKey}: ${error.message}`)
  }
  ...
  for (let x = 0; x < hashIds.length; x++) {
    ...
    if (!nodeCoreLookups[hashIdNode]) {
      ...
      let coreProof = coreProofResults[hashIdCore].proof
      ...
      results.push({
        hash_id_node: hashIdNode,
        proof: (requestedType === BASE64_MIME_TYPE) ? chpBinary.objectToBase64Sync(fullProof) : fullProof,
        anchors_complete: coreProofResults[hashIdCore].anchorsComplete || []
      })
    }
  }
