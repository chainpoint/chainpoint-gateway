...
async function getProofsByIDAsync(req, res, next) {
  ...
  for (let nodeProofDataItem of nodeProofDataItems) {
    ...
    let anchorsCompleteExists = result.anchors_complete != null && result.anchors_complete != undefined //not sure why this passes null test but is undefined
    if (anchorsCompleteExists && result.anchors_complete.length == 0) {
      event.ea = 'GetProofFail'
      analytics.sendEvent(event)
    } else if (anchorsCompleteExists && (result.anchors_complete.includes('btc')) || // eslint-disable-line
      result.anchors_complete.includes('tbtc')) { // eslint-disable-line
      event.ea = 'GetProofDataBtc'
      analytics.sendEvent(event)
    } else if (anchorsCompleteExists && (result.anchors_complete.includes('cal')) || // eslint-disable-line
      result.anchors_complete.includes('tcal')) { // eslint-disable-line
      event.ea = 'GetProofDataCal'
      analytics.sendEvent(event)
    }
  }
