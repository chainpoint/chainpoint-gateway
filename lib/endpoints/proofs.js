...
const db = require('./models/RocksDB.js')
...
async function getProofsByIDV1Async (req, res, next) {
  ...
  try {
    nodeProofDataItems = await db.getProofStateByHashIdNodeBatchAsync(hashIds)
  } catch (error) {
  ...
  let uniqueHashIdCores = nodeProofDataItems.reduce((result, val) => {
    if (!result.includes(val.hashIdCore)) result.push(val.hashIdCore)
    return result
  }, [])
  ...
  let results = []
  for (let [index, nodeProofDataItem] of nodeProofDataItems.entries()) {
    if (nodeProofDataItem === null) {
      // This hashIdNode was not found on the Node
      results.push({
        hash_id_node: hashIds[index],
        proof: null,
        anchors_complete: []
      })
    } else {
