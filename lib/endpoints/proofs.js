...
async function getProofsByIDAsync(req, res, next) {
  ...
  if (req.params && req.params.proof_id) {
    ...
    hashIds.push(req.params.proof_id)
  ...
  try {
    nodeProofDataItems = await rocksDB.getProofStatesBatchByProofIdsAsync(hashIds)
  } catch (error) {
...
function buildFullProof(coreProof, nodeProofDataItem) {
  ...
  fullProof.branch[0].label = 'aggregator'
  fullProof.branch[0].ops = nodeProofDataItem.proofState
  fullProof.branch[0].branches = coreBranches
  return fullProof
}
