...
async function getProofsByIDAsync(req, res, next) {
  ...
  if (hashIds.length > env.GET_PROOFS_MAX) {
  ...
  let uniqueCoreSubmissions = nodeProofDataItems.reduce((result, val) => {
    // if the node data item was not found for that hashIdNode, skip this item
    if (val.submission === null) return result
  ...
  try {
    ...
    console.error(`ERROR : Could not get proofs from Core : ${error.message}`)
  ...
  for (let nodeProofDataItem of nodeProofDataItems) {
    ...
    let fullProof = null
    if (coreProof) {
      fullProof = buildFullProof(hashIdNode, coreProof, nodeProofDataItem)
    }
...
function buildFullProof(hashIdNode, coreProof, nodeProofDataItem) {
