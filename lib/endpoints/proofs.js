...
async function getProofsByIDAsync(req, res, next) {
  ...
  // retrieve all the gatewayProofDataItems for the requested proofIds
  let gatewayProofDataItems = []
  try {
    gatewayProofDataItems = await rocksDB.getProofStatesBatchByProofIdsAsync(proofIds)
  } catch (error) {
    return next(new errors.InternalError('error retrieving Gateway proof data items'))
  }

  // convert all proof state value to chpv3
  gatewayProofDataItems = gatewayProofDataItems.map(gatewayProofDataItem => {
    if (gatewayProofDataItem.proofState === null) return gatewayProofDataItem
    gatewayProofDataItem.proofState = formatAsChainpointV3Ops(gatewayProofDataItem.proofState, 'sha-256')
    return gatewayProofDataItem
  })

  // get an array of all unique core submission objects from the proof data items by submitId
  let knownSubmitIds = []
  let uniqueCoreSubmissions = gatewayProofDataItems.reduce((result, val) => {
    // if the Gateway data item was not found for that proofId, skip this item
  ...
  for (let gatewayProofDataItem of gatewayProofDataItems) {
    let submitId = gatewayProofDataItem.submission ? gatewayProofDataItem.submission.submitId : null
    let coreProof = coreProofDataItems[submitId] ? coreProofDataItems[submitId].proof : null

    let fullProof = null
    if (coreProof) {
      fullProof = buildFullProof(coreProof, gatewayProofDataItem)
    }
    ...
    results.push({
      proof_id: gatewayProofDataItem.proofId,
      proof: proofResult,
...
function buildFullProof(coreProof, gatewayProofDataItem) {
  if (!coreProof || !gatewayProofDataItem) return null

  let fullProof = _.cloneDeep(coreProof)
  let coreBranches = _.cloneDeep(fullProof.branches)
  fullProof.proof_id = gatewayProofDataItem.proofId
  fullProof.hash = gatewayProofDataItem.hash
  fullProof.hash_received = utils.formatDateISO8601NoMs(new Date(parseInt(uuidTime.v1(gatewayProofDataItem.proofId))))
  fullProof.branches[0].label = 'aggregator'
  fullProof.branches[0].ops = gatewayProofDataItem.proofState
