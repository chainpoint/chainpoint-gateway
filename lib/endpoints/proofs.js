...
async function getProofsByIDV1Async (req, res, next) {
  ...
  try {
    ...
    return next(new restify.InternalError('error retrieving proof lookup data'))
  }
  ...
  for (let x = 0; x < hashIds.length; x++) {
    ...
    if (!nodeCoreLookups[hashIdNode]) {
      ...
      results.push({
        hash_id_node: hashIdNode,
        proof: (requestedType === BASE64_MIME_TYPE && fullProof) ? chpBinary.objectToBase64Sync(fullProof) : null,
...
async function getProofsFromCoreByHashIdsAsync (hashIdArray) {
  ...
  try {
    ...
    if (error.statusCode) throw new Error(`ERROR : Invalid response on GET proof : ${error.statusCode}`)
