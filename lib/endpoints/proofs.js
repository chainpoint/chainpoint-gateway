...
async function getProofsByIDAsync(req, res, next) {
  ...
  for (let nodeProofDataItem of nodeProofDataItems) {
    ...
    let anchorsCompleteExists = result && result.anchors_complete && Array.isArray(result.anchors_complete) //not sure why this passes null test but is undefined
    if (fullProof) {
      try {
        logger.info(`Proof Size: ${JSON.stringify(fullProof).length}`)
        logger.info(`Proof: ${JSON.stringify(fullProof)}`)
      } catch (error) {
        logger.error(`could not get proof size of proof ${result.proof_id}`)
      }
      event.ea = 'GetProofSuccess'
      analytics.sendEvent(event)
      if (anchorsCompleteExists) {
        if (result.anchors_complete.includes('btc') || result.anchors_complete.includes('tbtc')) { // eslint-disable-line
          event.ea = 'GetProofSuccessBtc'
          analytics.sendEvent(event)
        } else if (result.anchors_complete.includes('cal') || result.anchors_complete.includes('tcal')) { // eslint-disable-line
