...
async function getProofsByIDV1Async(req, res, next) {
  ...
  if (req.params && req.params.hash_id_node) {
    ...
    if (!uuidValidate(req.params.hash_id_node, 1)) {
      return next(
        new errors.InvalidArgumentError('invalid request, bad hash_id')
      )
    }
  ...
  if (hashIds.length === 0) {
    return next(
      new errors.InvalidArgumentError(
        'invalid request, at least one hash id required'
      )
    )
  }
  ...
  if (hashIds.length > env.GET_PROOFS_MAX_REST) {
    return next(
      new errors.InvalidArgumentError(
        'invalid request, too many hash ids (' +
          env.GET_PROOFS_MAX_REST +
          ' max)'
      )
    )
  }
  ...
  let requestedType =
    req.accepts(JSONLD_MIME_TYPE) && !req.accepts(BASE64_MIME_TYPE)
      ? JSONLD_MIME_TYPE
      : BASE64_MIME_TYPE
  ...
  try {
    nodeProofDataItems = await rocksDB.getProofStatesBatchByHashIdNodesAsync(
      hashIds
    )
  } catch (error) {
    return next(
      new errors.InternalError('error retrieving node proof data items')
    )
  }
  ...
  nodeProofDataItems = nodeProofDataItems.map(nodeProofDataItem => {
    ...
    nodeProofDataItem.proofState = formatAsChainpointV3Ops(
      nodeProofDataItem.proofState,
      'sha-256'
    )
  ...
  let uniqueHashIdCores = nodeProofDataItems.reduce((result, val) => {
    if (!result.includes(val.hashIdCore) && val.hashIdCore != null)
      result.push(val.hashIdCore)
    return result
  }, [])
  ...
  try {
    coreProofDataItems = await cachedProofs.getCachedCoreProofsAsync(
      uniqueHashIdCores
    )
  } catch (error) {
    console.error(
      `ERROR : Could not get proofs from proofs.chainpoint.org : ${
        error.message
      }`
    )
    return next(
      new errors.InternalError(
        'error retrieving proofs from proofs.chainpoint.org'
      )
    )
  }

  // assemble all Core proofs received into an object keyed by hash_id_core
  coreProofDataItems = coreProofDataItems.reduce(
    (results, coreProofDataItem) => {
  ...
  for (let nodeProofDataItem of nodeProofDataItems) {
    ...
    let coreProof = coreProofDataItems[hashIdCore]
      ? coreProofDataItems[hashIdCore].proof
      : null

    let fullProof = null
    if (coreProof) {
      fullProof = buildFullProof(
        hashIdNode,
        hashIdCore,
        coreProof,
        nodeProofDataItem
    ...
    results.push({
      ...
      proof: proofResult,
      anchors_complete: coreProofDataItems[hashIdCore]
        ? coreProofDataItems[hashIdCore].anchorsComplete
        : []
    })
  }
