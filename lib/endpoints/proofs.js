...
async function getProofsByIDAsync(req, res, next) {
  ...
  for (let nodeProofDataItem of nodeProofDataItems) {
    ...
    if (result.anchors_complete && result.anchors_complete.length == 0) {
      event.ea = 'GetProofFail'
      analytics.sendEvent(event)
    } else if (
      (result.anchors_complete && result.anchors_complete.includes('btc')) ||
      result.anchors_complete.includes('tbtc')
    ) {
      event.ea = 'GetProofDataBtc'
      analytics.sendEvent(event)
    } else if (
      (result.anchors_complete && result.anchors_complete.includes('cal')) ||
      result.anchors_complete.includes('tcal')
    ) {
