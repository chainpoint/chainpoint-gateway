...
async function getConfigInfoV1Async (req, res, next) {
  ...
  // Fetch Core Config data to be able to provide callers of this endpoint with the Calendar Height of Core
  let coreConfig = await rp({ uri: 'https://a.chainpoint.org/config', json: true })
  ...
  res.send({
    ...
    calendar: {
      ...
      coreHeight: coreConfig.calendar.height,
