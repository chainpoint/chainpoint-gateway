...
async function validateSignedDataAsync (signedData) {
  let prefixedSigValues = signedData.split(':')
  let pubKeyHash = prefixedSigValues[0]
  let sig = prefixedSigValues[1]
  console.log('====================================')
  console.log(pubKeyHash, sig, 'validateSignedDataAsync')
  console.log('====================================')
  ...
  let isValidSig = await publicKeys.validateSignatureAsync(objecthash(signedData.data).toString('hex'), publicKeySet[pubKeyHash], sig)
  if (!isValidSig) {
...
async function getConfigInfoV1Async (req, res, next) {
  ...
  let dataFromCore = (req.headers && req.headers.data) ? JSON.parse(req.headers.data) : {}

  try {
    if (dataFromCore) {
      console.log('INFO : Config : Core Data has been pushed to node')
      // Verify the signed data, if valid, persist to redis. If signature doesn't match discard the 'data'
      let signatureValidation = await validateSignedDataAsync(dataFromCore.sig)
      console.log('====================================')
