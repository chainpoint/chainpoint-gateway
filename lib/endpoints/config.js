...
async function validateSignatureAsync (signedData) {
  ...
  let isValidSig = await publicKeys.validateSignatureAsync(signedData, publicKeySet[pubKeyHash], sig)
  if (!isValidSig) {
...
async function getConfigInfoV1Async (req, res, next) {
  ...
  try {
    if (dataFromCore) {
      ...
      let signatureValidation = await validateSignatureAsync(dataFromCore.sig)
      console.log('====================================')
