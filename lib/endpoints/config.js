  ...
  try {
    if (!isEmpty(dataFromCore)) {
      ...
      if (signatureValidation) {
        ...
        await rocksDB.setAsync('dataFromCore', JSON.stringify(dataFromCore.data))
        ...
        let lastChallengeResponse = (await rocksDB.getAsync('challenge_response')) || []
        ...
        if (lastAudit) {
          ...
          let lastAuditChecksVerboseOutput = (function (publicIpPass) {
            if (publicIpPass) {
              return ['ntp-time', 'cal-state', 'node-version'].map((currVal) => {
          ...
          console.log(`INFO: Audit : audit ${lastAuditChecks.audit} (tnt-balance: ${lastAuditChecks['tnt-balance']}, public-ip: ${lastAuditChecks['public-ip']}${lastAuditChecksVerboseOutput})`)
        }
      } else {
