  ...
  try {
    if (!isEmpty(dataFromCore)) {
      ...
      if (signatureValidation) {
        ...
        let [
          lastChallengeCreateTime = 0,
          lastNodeChallengeResponse = ''
        ] = lastChallengeResponse.split(':') // eslint-disable-line
        ...
        if (lastAudit) {
          let lastAuditChecks = {
            ...
            ...(lastAudit.public_ip_pass
              ? { 'ntp-time': lastAudit.time_pass ? 'pass' : 'fail' }
              : {}),
            ...(lastAudit.public_ip_pass
              ? { 'cal-state': lastAudit.cal_state_pass ? 'pass' : 'fail' }
              : {}),
            ...(lastAudit.public_ip_pass
              ? {
                  'node-version': lastAudit.node_version_pass ? 'pass' : 'fail'
                }
              : {})
          }
