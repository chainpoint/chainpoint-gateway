...
let lnd = function(socket, network, unlockOnly = false, inHostContext = false) {
  ...
  LND_DIR = `${process.env.HOME}/${inHostContext ? '.chainpoint/node/' : ''}.lnd`
  LND_SOCKET = socket
  LND_CERTPATH = `${LND_DIR}/tls.cert`
  LND_MACAROONPATH = `${LND_DIR}/data/chain/bitcoin/${network}/admin.macaroon`
  if (unlockOnly) {
          ...
          if (!IS_UNLOCKING) {
            ...
            try {
              await lnRPCNodeClient
                .unlocker()
                .unlockWalletAsync({ wallet_password: hotWalletPass || env.HOT_WALLET_PASS })
            } catch (error) {
