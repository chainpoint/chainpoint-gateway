...
const nodeProofDataItemSchema = new JSBinaryType({
  hashIdNode: 'Buffer',
  hash: 'Buffer',
  proofState: [{ 'left?': 'Buffer', 'right?': 'Buffer' }],
  hashIdCore: 'Buffer'
})
...
function encodeProofStateValue (nodeProofDataItem) {
  let stateObj = {
    ...
    proofState: nodeProofDataItem.proofState.map((op) => {
      if (op.left) {
        return { left: Buffer.from(op.left, 'hex') }
      } else if (op.right) {
        return { right: Buffer.from(op.right, 'hex') }
      }
    }),
...
function decodeProofStateValue (proofStateValue) {
  ...
  let nodeProofDataItem = {
    ...
    proofState: stateObj.proofState.map((op) => {
      if (op.left) {
        return { left: op.left.toString('hex') }
      } else if (op.right) {
        return { right: op.right.toString('hex') }
      }
    }),
