...
function createBinaryReputationItemIdIndexKey(id) {
  let idBuffer = Buffer.alloc(8)
  idBuffer.writeDoubleBE(id)
  return Buffer.concat([prefixBuffers.REP_ITEM_ID_INDEX, idBuffer])
}
...
function getIdFromProofValueKey(key) {
...
function encodeReputationItemValue(reputationItem) {
  ...
  let repObj = {
    ...
    signature: Buffer.from(reputationItem.signature.substring(2), 'hex') // omit the leading `0x`
  }
...
function decodeReputationItemValue(reputationItemValue) {
  ...
  let reputationItem = {
    ...
    signature: `0x${repObj.signature.toString('hex')}`
  }
...
async function getReputationItemByIdAsync(id) {
  try {
    let indexKey = createBinaryReputationItemIdIndexKey(id)
    ...
    if (error.notFound) {
      ...
      let err = `Error reading reputation item by id : ${error.message}`
      throw err
    }
  }
}

async function getReputationItemsRangeByIdsAsync(minId, maxId) {
  return new Promise(async (resolve, reject) => {
    let results = []
    let minIndexKey = createBinaryReputationItemIdIndexKey(minId)
    let maxIndexKey = createBinaryReputationItemIdIndexKey(maxId)
    let minKey, maxKey
    try {
...
async function saveReputationItemAsync(reputationItem) {
  ...
  let reputationItemIdIndexKey = createBinaryReputationItemIdIndexKey(reputationItem.id)
  ...
  ops.push({ type: 'put', key: reputationItemIdIndexKey, value: reputationItemValueKey })
  try {
    await db.batch(ops)
  } catch (error) {
...
async function saveReputationItemProofAsync(repId, proofString) {
  let reputationItemProofValueKey = createBinaryReputationItemProofValueKey(repId)
...
async function getReputationItemProofByRepIdAsync(repId) {
  try {
    let key = createBinaryReputationItemProofValueKey(repId)
    ...
    if (error.notFound) {
      ...
      let err = `Error reading reputation item proof by repId : ${error.message}`
      throw err
    }
  }
}

async function getReputationItemProofsRangeByRepIdsAsync(minRepId, maxRepId) {
  return new Promise(async (resolve, reject) => {
    let results = []
    let minValueKey = createBinaryReputationItemProofValueKey(minRepId)
    let maxValueKey = createBinaryReputationItemProofValueKey(maxRepId)
    db.createReadStream({ gte: minValueKey, lte: maxValueKey })
      .on('data', async data => {
        results.push({
          id: getIdFromProofValueKey(data.key),
...
module.exports = {
  ...
  getReputationItemByIdAsync: getReputationItemByIdAsync,
  getReputationItemsRangeByIdsAsync: getReputationItemsRangeByIdsAsync,
  saveReputationItemAsync: saveReputationItemAsync,
  saveReputationItemProofAsync: saveReputationItemProofAsync,
  getReputationItemProofByRepIdAsync: getReputationItemProofByRepIdAsync,
  getReputationItemProofsRangeByRepIdsAsync: getReputationItemProofsRangeByRepIdsAsync,
  setAsync: setAsync,
  getAsync: getAsync,
