...
const gatewayProofDataItemSchema = new JSBinaryType({
  proofId: 'Buffer',
  hash: 'Buffer',
  proofState: ['Buffer'],
  submission: {
...
function createBinaryProofStateValueKey(proofId) {
  let uuidBuffer = Buffer.from(proofId.replace(/-/g, ''), 'hex')
...
function encodeProofStateValue(gatewayProofDataItem) {
  let stateObj = {
    proofId: Buffer.from(gatewayProofDataItem.proofId.replace(/-/g, ''), 'hex'),
    hash: Buffer.from(gatewayProofDataItem.hash, 'hex'),
    proofState: gatewayProofDataItem.proofState,
    submission: {
      submitId: Buffer.from(gatewayProofDataItem.submission.submitId.replace(/-/g, ''), 'hex'),
      cores: gatewayProofDataItem.submission.cores.map(core => {
        return { ip: core.ip, proofId: Buffer.from(core.proofId.replace(/-/g, ''), 'hex') }
      })
    }
  }
  return gatewayProofDataItemSchema.encode(stateObj)
}

function decodeProofStateValue(proofStateValue) {
  let leftCode = Buffer.from('\x00')
  let rightCode = Buffer.from('\x01')
  let stateObj = gatewayProofDataItemSchema.decode(proofStateValue)
...
async function getProofStatesBatchByProofIdsAsync(proofIds) {
  ...
  for (let proofId of proofIds) {
    try {
      ...
      let gatewayProofDataItem = decodeProofStateValue(proofStateValue)
      results.push(gatewayProofDataItem)
    } catch (error) {
      if (error.notFound) {
        results.push({
          proofId: proofId,
...
async function saveProofStatesBatchAsync(gatewayProofDataItems) {
  let ops = []

  for (let gatewayProofDataItem of gatewayProofDataItems) {
    let proofStateValueKey = createBinaryProofStateValueKey(gatewayProofDataItem.proofId)
    let proofStateTimeIndexKey = createBinaryProofStateTimeIndexKey()
    let proofStateValue = encodeProofStateValue(gatewayProofDataItem)
