...
async function updateCountMetrics (metricObjects) {
  for (let metricObject of metricObjects) {
    try {
      ...
      let err = `Unable to write metric : ${metricObject.key} ${metricObject.value}: ${error.message}`
      throw err
    }
  }
}
...
async function updateRecentHashDataAsync (recentHashDataObjects) {
  try {
    let recentHashDataBinaryKey = Buffer.from('nodestats:hashes:last25')
    let recentHashDataValue = encodeRecentHashDataValue(recentHashDataObjects)
    await db.put(recentHashDataBinaryKey, recentHashDataValue)
  } catch (error) {
...
setInterval(pruneOldProofStateDataAsync, PRUNE_INTERVAL_SECONDS * 1000)
...
module.exports = {
  ...
  updateCountMetrics: updateCountMetrics,
  updateRecentHashDataAsync: updateRecentHashDataAsync
}
