...
async function savePublicKeysAsync (publicKeyItems) {
  ...
  for (let publicKeyItem of publicKeyItems) {
    let publicKeyValueKey = createPublicKeyValueKey(publicKeyItem.pubKeyHash)
...
async function getAllPublicKeysAsync () {
  return new Promise((resolve, reject) => {
    let results = []
    let minKey = createPublicKeyValueKeyMin()
    let maxKey = createPublicKeyValueKeyMax()
