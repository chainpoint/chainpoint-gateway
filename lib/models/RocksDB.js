...
const prefixBuffers = {
  PROOF_STATE_INDEX: Buffer.from('c2a1', 'hex'),
  PROOF_STATE_VALUE: Buffer.from('c2a2', 'hex'),
  INCOMING_HASH_OBJECTS: Buffer.from('c2b1', 'hex'),
  HMAC_KEY_VALUE: Buffer.from('a1', 'hex'),
  PUBLIC_KEY_VALUE: Buffer.from('c1', 'hex'),
  CALENDAR_BLOCK_VALUE: Buffer.from('b1', 'hex'),
  CALENDAR_BLOCK_CAL_DATAID_INDEX: Buffer.from('b2', 'hex'),
  CALENDAR_BLOCK_BTCC_DATAID_INDEX: Buffer.from('b3', 'hex')
}
...
function createBinaryCalendarBlockTypeDataIdIndexKeyByTypeAndDataId(
  type,
  dataId
) {
...
function decodeCalendarBlockValue(calendarBlockValue) {
  ...
  let calendarBlock = {
    ...
    sig: `${calendarBlockObj.sig.pubKeyPrefix.toString(
      'hex'
    )}:${calendarBlockObj.sig.value.toString('base64')}`
  }
...
function decodeCalendarBlockTypeDataIdIndexValue(type, indexValue) {
  ...
  switch (type) {
    case 'cal':
      result.hash = indexValue.toString('hex')
      break
    case 'btc-c':
      result.dataVal = indexValue.toString('hex')
      break
    default:
      throw new Error('Unknown type for CalendarBlockTypeDataIdIndexValue')
  }
...
async function saveCalendarBlockBatchAsync(calendarBlocks) {
  ...
  for (let calendarBlock of calendarBlocks) {
    ...
    let calendarBlockValueKey = createBinaryCalendarBlockValueKeyByHeight(
      calendarBlock.id
    )
    let calendarBlockValue = encodeCalendarBlockValue(calendarBlock)
    ops.push({
      type: 'put',
      key: calendarBlockValueKey,
      value: calendarBlockValue
    })
    // if cal or btc-c blocks, index appropriate verification value
    if (calendarBlock.type === 'cal' || calendarBlock.type === 'btc-c') {
      let calendarBlockTypeDataIdIndexKey = createBinaryCalendarBlockTypeDataIdIndexKeyByTypeAndDataId(
        calendarBlock.type,
        calendarBlock.dataId
      )
      let calendarBlockTypeDataIdIndexValue = encodeCalendarBlockTypeDataIdIndexValue(
        calendarBlock
      )
      ops.push({
        type: 'put',
        key: calendarBlockTypeDataIdIndexKey,
        value: calendarBlockTypeDataIdIndexValue
      })
    }
  }
  if (ops.length > 0) {
    let calendarTopBlockHeightValue = encodeCalendarTopBlockHeightValue(
      topBlockHeight
    )
    ops.push({
      type: 'put',
      key: CALENDAR_TOP_BLOCK_BINARY_KEY,
      value: calendarTopBlockHeightValue
    })
  }
...
async function getTopCalendarBlockAsync() {
  try {
    let topCalendarBlockHeightValue = await db.get(
      CALENDAR_TOP_BLOCK_BINARY_KEY
    )
    let topCalendarBlockHeight = decodeCalendarTopBlockHeightValue(
      topCalendarBlockHeightValue
    )
    let topCalendarBlockValue = await db.get(
      createBinaryCalendarBlockValueKeyByHeight(topCalendarBlockHeight)
    )
...
async function getCalendarBlockByHeightAsync(blockHeight) {
  try {
    let calendarBlockValue = await db.get(
      createBinaryCalendarBlockValueKeyByHeight(blockHeight)
    )
    ...
    if (error.notFound) {
      ...
      let err = `Unable to read calendar block at height ${blockHeight} : ${
        error.message
      }`
      throw err
    }
...
async function getCalendarBlockRangeAsync(startIndex, endIndex) {
  return new Promise((resolve, reject) => {
    ...
    db.createReadStream({ gte: minKey, lte: maxKey })
      .on('data', data => {
        let block = decodeCalendarBlockValue(data.value)
        blocks.push(block)
      })
      .on('error', error => {
        let err = `Error reading calendar blocks for range ${startIndex} to ${endIndex} : ${
          error.message
        }`
        return reject(err)
      })
...
async function getCalBlockByDataIdAsync(dataId) {
  try {
    let calendarBlockTypeDataIdIndexValue = await db.get(
      createBinaryCalendarBlockTypeDataIdIndexKeyByTypeAndDataId('cal', dataId)
    )
    let indexValue = decodeCalendarBlockTypeDataIdIndexValue(
      'cal',
      calendarBlockTypeDataIdIndexValue
    )
    return indexValue
  } catch (error) {
    if (error.notFound) {
      ...
      let err = `Unable to read calendar cal block index value for dataId ${dataId} : ${
        error.message
      }`
      throw err
    }
...
async function getBtcCBlockByDataIdAsync(dataId) {
  try {
    let calendarBlockTypeDataIdIndexValue = await db.get(
      createBinaryCalendarBlockTypeDataIdIndexKeyByTypeAndDataId(
        'btc-c',
        dataId
      )
    )
    ...
    if (error.notFound) {
      ...
      let err = `Unable to read calendar btc-c block index value for dataId ${dataId} : ${
        error.message
      }`
      throw err
    }
...
function createBinaryProofStateTimeIndexKey() {
  ...
  return Buffer.concat([
    prefixBuffers.PROOF_STATE_INDEX,
    timestampBuffer,
    rndBuffer
  ])
}
...
function encodeProofStateValue(nodeProofDataItem) {
  let stateObj = {
    hashIdNode: Buffer.from(
      nodeProofDataItem.hashIdNode.replace(/-/g, ''),
      'hex'
    ),
    hash: Buffer.from(nodeProofDataItem.hash, 'hex'),
    proofState: nodeProofDataItem.proofState,
    hashIdCore: Buffer.from(
      nodeProofDataItem.hashIdCore.replace(/-/g, ''),
      'hex'
    )
  }
...
function decodeProofStateValue(proofStateValue) {
  ...
  let nodeProofDataItem = {
    ...
    proofState: stateObj.proofState.reduce((result, op, index, proofState) => {
      if (op.equals(leftCode))
        result.push({ left: proofState[index + 1].toString('hex') })
      if (op.equals(rightCode))
        result.push({ right: proofState[index + 1].toString('hex') })
      return result
    }, []),
  ...
  for (let hashIdNode of hashIdNodes) {
    try {
      ...
      if (error.notFound) {
        ...
        let err = `Unable to read proof state for hash with hashIdNode = ${hashIdNode} : ${
          error.message
        }`
        throw err
      }
...
async function saveProofStatesBatchAsync(nodeProofDataItems) {
  ...
  for (let nodeProofDataItem of nodeProofDataItems) {
    let proofStateValueKey = createBinaryProofStateValueKey(
      nodeProofDataItem.hashIdNode
    )
    ...
    ops.push({
      type: 'put',
      key: proofStateTimeIndexKey,
      value: proofStateValueKey
    })
  }
...
async function pruneProofStateDataSince(timestampMS) {
  return new Promise((resolve, reject) => {
    ...
    db.createReadStream({ gt: minKey, lte: maxKey })
      ...
      .on('error', error => {
        let err = `Error reading proof state keys for pruning : ${
          error.message
        }`
        return reject(err)
      })
...
async function pruneOldProofStateDataAsync() {
  ...
  try {
    ...
    console.error(
      `ERROR : An error occurred during proof state pruning : ${error.message}`
    )
  }
}
...
function createBinaryIncomingHashObjectsTimeIndexKey() {
  ...
  let rndBuffer = crypto.randomBytes(16)
  return Buffer.concat([
    prefixBuffers.INCOMING_HASH_OBJECTS,
    timestampBuffer,
    rndBuffer
  ])
}
...
function encodeIncomingHashObjectsValue(hashObjects) {
  hashObjects = hashObjects.map(hashObject => {
    return {
      hash_id_node: Buffer.from(
        hashObject.hash_id_node.replace(/-/g, ''),
        'hex'
      ),
...
async function getIncomingHashesUpToAsync(maxTimestamp) {
  return new Promise((resolve, reject) => {
    ...
    let maxIncomingHashObjectsBinaryKey = createBinaryIncomingHashObjectsTimeIndexMax(
      maxTimestamp
    )
    db.createReadStream({
        gt: minIncomingHashObjectsBinaryKey,
        lte: maxIncomingHashObjectsBinaryKey
      })
      .on('data', async data => {
      ...
      .on('end', async () => {
        return resolve([hashesObjects, delOpts])
      })
  })
}
...
async function getCountMetricsAsync() {
  return new Promise((resolve, reject) => {
    ...
    db.createReadStream({
      gt: minNodeStatsCounterBinaryKey,
      lt: maxNodeStatsCounterBinaryKey
    })
      .on('data', async data => {
        counters.push({
...
async function saveCountMetricsAsync(metricObjects, prunedKeys) {
  ...
  for (let metricObject of metricObjects) {
    try {
      ...
      let err = `Unable to write counter : ${metricObject.key} ${
        metricObject.value
      }: ${error.message}`
      throw err
    }
  ...
  if (prunedKeys.length > 0) {
    let delOpts = prunedKeys.map(key => {
...
function decodeRecentHashDataValue(recentHashDataObjectsBinary) {
  ...
  let recentHashDataObjects = recentHashDataSchema.decode(
    recentHashDataObjectsBinary
  )
  recentHashDataObjects = recentHashDataObjects.map(hashDataItem => {
    return {
...
async function deleteBatchAsync(delOpts) {
  return db.batch(delOpts)
}
...
async function savePublicKeysAsync(publicKeyItems) {
  ...
  for (let publicKeyItem of publicKeyItems) {
    let publicKeyValueKey = createBinaryPublicKeyValueKey(
      publicKeyItem.pubKeyHash
    )
...
async function getAllPublicKeysAsync() {
  return new Promise((resolve, reject) => {
    ...
    db.createReadStream({ gte: minKey, lte: maxKey })
      .on('data', async data => {
        results.push({
          pubKeyHash: data.key.slice(1).toString('hex'),
