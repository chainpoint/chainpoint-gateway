...
async function coreRequestAsync(options, coreIP, retryCount = 3, headers = false) {
  ...
  // lsat related requests need headers and status from response
  // so we send the whole thing
...
async function submitHashAsync(hash) {
  ...
  for (let coreIP of env.CHAINPOINT_CORE_CONNECT_IP_LIST) {
    try {
      ...
      let submitResponse = await coreRequestAsync(postHashOptions, coreIP, 0, true)
      ...
      let paidInvoice
      payInvoiceAsync(decodedPaymentRequest, submitHashInvoiceId).then(response => {
        paidInvoice = response
        logger.info(
      ...
      submitResponse = await coreRequestAsync(postHashOptions, coreIP, 5)

      logger.info(
        `Aggregator : Hash submitted to Core ${coreIP} : invoiceId : ${submitHashInvoiceId} : ${invoiceAmount}`
      )

      if (!paidInvoice) throw new Error(`Aggregator: Invoice ${submitHashInvoiceId} was never settled.`)
