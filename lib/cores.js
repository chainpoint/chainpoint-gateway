...
async function submitHashAsync(hash, token) {
  let postHashoptions = buildRequestOptions(null, 'POST', '/hashes', { hash: hash, usageToken: token })
...
async function getProofsAsync(coreIP, hashIdCores) {
  let getProofsOptions = buildRequestOptions({ hashids: hashIdCores.join(',') }, 'GET', '/proofs', null, 5000)
...
async function refreshUsageTokenAsync(activeToken) {
  ...
  if (lastError.statusCode) throw new Error(`Invalid response on POST /usagetoken/refresh : ${lastError.statusCode}`)
  throw new Error('Invalid response on POST /usagetoken/refresh')
}

async function getETHStatsAsync(nodeETHAddress) {
  // build the request to Core
  let ethStatsOptions = buildRequestOptions(null, 'GET', `/ethstats/${nodeETHAddress}`)

  let lastError = null
  for (let coreIP of CONNECTED_CORE_IPS) {
    try {
      let ethStatsResult = await coreRequestAsync(ethStatsOptions, coreIP)
      if (ethStatsResult[nodeETHAddress]) return ethStatsResult[nodeETHAddress]
    } catch (error) {
      // Record most recent error, ignore, and try next coreIP
      lastError = error
    }
  }
  if (lastError.statusCode) throw new Error(`Invalid response on GET /ethstats/{address} : ${lastError.statusCode}`)
  throw new Error('Invalid response on GET /ethstats/{address}')
}

async function purchaseCreditsAsync(rawTransaction) {
  // build the request to Core
  let purchaseCreditsOptions = buildRequestOptions(null, 'POST', `/usagetoken/credit`, { tx: rawTransaction }, 60000)

  let lastError = null
  for (let coreIP of CONNECTED_CORE_IPS) {
    try {
      let purchaseCreditsResult = await coreRequestAsync(purchaseCreditsOptions, coreIP)
      if (purchaseCreditsResult.usageToken) return purchaseCreditsResult.usageToken
    } catch (error) {
      // Record most recent error, ignore, and try next coreIP
      lastError = error
    }
  }
  if (lastError.statusCode) throw new Error(`Invalid response on POST /usagetoken/credit : ${lastError.statusCode}`)
