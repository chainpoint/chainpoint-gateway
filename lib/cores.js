...
async function connectAsync() {
  ...
  if (connectedCoreIPResult.envMismatch)
    logger.warn(
      `Unable to connect to Cores with a different environment setting. This Node is set to '${env.NODE_ENV}'`
    )
...
async function getConnectedCoreIPsAsync(coreIPList, coreConnectionCount) {
  ...
  for (let coreIP of coreIPList) {
    try {
      ...
      let envMatch = coreResponse.environment === env.NODE_ENV
      ...
      if (!envMatch) envMismatch = true
      if (!modeMatch) modeMismatch = true
      if (envMatch && modeMatch && isCoreSynched) connectedCoreIPs.push(coreIP)
    } catch (error) {
  ...
  return { connected: connectedCoreIPs.length >= coreConnectionCount, ips: connectedCoreIPs, envMismatch, modeMismatch }
}
...
async function coreRequestAsync(options, coreIP, retryCount = 3) {
  ...
  await retry(
    ...
    },
    {
      ...
      randomize: true,
      onRetry: error => {
        logger.warn(`Core request : ${error.statusCode || 'no response'} : ${error.message} : retrying`)
      }
    }
  )
