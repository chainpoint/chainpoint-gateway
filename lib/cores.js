...
async function connectAsync() {
  ...
  if (!CONNECTED_CORE_IPS.length || CONNECTED_CORE_IPS.length < coreConnectionCount)
...
async function broadcastEthTxAsync(rawTransaction) {
  ...
  try {
    ...
    throw new Error(`Invalid response on POST /eth/broadcast : ${error.statusCode} : ${error.message}`)
  }
}
...
async function updateUsageTokenAudienceAsync(activeToken) {
  ...
  if (lastError.statusCode) throw new Error(`Invalid response on POST /usagetoken/audience : ${lastError.message}`)
