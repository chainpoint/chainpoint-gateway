...
let lnService = require('ln-service')
const logger = require('./logger.js')
const openChannelToCore = require('../cli/scripts/1_lnd_payment_channels')
...
async function connectAsync(opts) {
  // Set Number of Cores to connect to and establish payment channels with. This is provided via interactive cli scripts
  coreConnectionCount = opts['CORE_PAYMENT_CHANNEL_COUNT']

  // Retrieve the list of Core IPs we can work with
  let coreIPList = []
  if (!_.isEmpty(opts['CONNECTED_CORE_PAYMENT_CHANNELS_IPS'])) {
    // Core IPs have been supplied in CHAINPOINT_CORE_CONNECT_IP_LIST, use those IPs only
    coreIPList = opts['CONNECTED_CORE_PAYMENT_CHANNELS_IPS']
  } else {
  ...
  let connectedCoreIPResult = await getConnectedCoreIPsAsync(coreIPList, coreConnectionCount, {
    satoshis: opts['SATOSHIS_PER_CORE_PAYMENT_CHANNEL']
  })
...
async function getConnectedCoreIPsAsync(coreIPList, coreConnectionCount, lndOpts = {}) {
  ...
  for (let coreIP of coreIPList) {
    try {
      ...
      if (networkMatch && isCoreSynched) {
        await openChannelToCore({ pubkey: coreResponse.pubkey, satoshis: lndOpts.satoshis })

        connectedCoreIPs.push(coreIP)
      }
    } catch (error) {
...
async function submitHashAsync(hash) {
  ...
  for (let coreIP of CONNECTED_CORE_IPS) {
    try {
      ...
      let decodedPaymentRequest = await lnService.decodePaymentRequest({ lnd, request: invoiceResponse.invoice })
      let submitHashInvoiceId = decodedPaymentRequest.description.split(':')[1]
      logger.info(`Invoice received from Core ${coreIP} : SubmitHashInvoiceId = ${submitHashInvoiceId}`)
      // ensure that the invoice amount does not exceed max payment amount
      if (decodedPaymentRequest.tokens > env.MAX_SATOSHI_PER_HASH)
        throw new Error(
          `Invoice amount of ${decodedPaymentRequest.tokens} exceeds MAX_SATOSHI_PER_HASH of ${
            env.MAX_SATOSHI_PER_HASH
          } : SubmitHashInvoiceId = ${submitHashInvoiceId} `
        )
      // pay the invoice
      await payInvoiceAsync(invoiceResponse.invoice, submitHashInvoiceId)
      logger.info(`Invoice paid to Core ${coreIP} : SubmitHashInvoiceId = ${submitHashInvoiceId}`)
      ...
      logger.info(
        `Hash submitted to Core ${coreIP} : SubmitHashInvoiceId = ${submitHashInvoiceId} : Hash = ${hash} : HashID = ${
          submitResponse.hash_id
        }`
      )
      responses.push({
        ip: coreIP,
...
async function payInvoiceAsync(invoice, submitHashInvoiceId) {
  try {
    let payResponse = await lnService.pay({ lnd, request: invoice })
    return payResponse
  } catch (error) {
    throw new Error(`Error paying invoice : SubmitHashInvoiceId = ${submitHashInvoiceId} : ${error.message || error}`)
  }
}
