...
let { lnd } = lnService.authenticatedLndGrpc({
  ...
  macaroon: path.resolve(homedir, '.lnd/data/chain/bitcoin/testnet/admin.macaroon'), // env.LND_MACAROON,
  ...
  for (let coreIP of coreIPList) {
    try {
      ...
      if (networkMatch && isCoreSynched) {
        await openChannelToCore({ pubkey: coreResponse.public_key, satoshis: lndOpts.satoshis })
