...
async function submitHashAsync(hash, token) {
  ...
  for (let coreIP of CONNECTED_CORE_IPS) {
    try {
      ...
      logger.warn(
        `submitHashAsync : Unable to connect to Core ${coreIP} : ${JSON.stringify(postHashoptions)} : Hash ${hash} : ${
          error.message
        }`
      )
    }
...
async function getProofsAsync(coreIP, hashIdCores) {
  ...
  try {
    let coreResponse = await coreRequestAsync(getProofsOptions, coreIP, 1)
    return coreResponse
  } catch (error) {
    if (error.statusCode) throw new Error(`Invalid response on GET proof : ${error.statusCode} : ${error.message}`)
    throw new Error(`Invalid response received on GET proof ${error.message}`)
  }
}
