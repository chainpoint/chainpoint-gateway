...
async function connectAsync(coreConnectionCount) {
  ...
  if (env.CHAINPOINT_CORE_CONNECT_IP_LIST === '') {
    ...
    coreIPList = env.CHAINPOINT_CORE_CONNECT_IP_LIST.split(',')
  }
  ...
  try {
    ...
    if (CONNECTED_CORE_IPS.length < coreConnectionCount)
      throw `Unable to connect to ${coreConnectionCount} Cores as required`
  } catch (error) {
    throw `Unable to establish connection to Core`
  }
...
async function getProofsAsync(coreIP, hashIdCores) {
  ...
  try {
    // TODO: Handle retrieval from redundant Core IPs
