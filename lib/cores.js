...
async function submitHashAsync(hash) {
  let postHashoptions = buildRequestOptions(null, 'POST', '/hashes', { hash: hash })
...
async function getLatestCalBlockHashAsync() {
  let getStatusOptions = buildRequestOptions(null, 'GET', '/status')
...
function buildRequestOptions(headerValues, method, uri, body, timeout = 3000) {
  return {
    headers: Object.assign({ 'Content-Type': 'application/json' }, headerValues),
    method: method,
    uri: uri,
    body: body || undefined,
    json: true,
...
module.exports = {
  ...
  getLatestCalBlockHashAsync: getLatestCalBlockHashAsync,
