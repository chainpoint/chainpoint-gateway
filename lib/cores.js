...
async function connectAsync(opts) {
  console.log('====================================')
  console.log('connectAsync -> opts', JSON.stringify(opts))
  console.log('====================================')

  // Initialize the Lightning grpc object
  let { lnd } = lnService.authenticatedLndGrpc({
    cert: opts['LND_TLS_CERT'],
    macaroon: opts['LND_MACAROON'],
  ...
  let coreConnectionCount = opts['CORE_PAYMENT_CHANNEL_COUNT']
  ...
  if (!_.isEmpty(opts['CONNECTED_CORE_PAYMENT_CHANNELS_IPS'])) {
    ...
    coreIPList = opts['CONNECTED_CORE_PAYMENT_CHANNELS_IPS'].split(',').map(currVal => currVal.split(':')[0])
  } else {
  ...
  let CONNECTED_CORE_IPS = connectedCoreIPResult.ips
...
async function getConnectedCoreIPsAsync(coreIPList, coreConnectionCount, lndOpts = {}) {
  ...
  for (let coreIP of coreIPList) {
    try {
      ...
      await retry(
        async () => {
          coreResponse = await coreRequestAsync(getStatusOptions, coreIP, 0)
        },
        {
      ...
      if (networkMatch && isCoreSynched) {
        await openChannelToCore({
          pubkey: coreResponse.identity_pubkey,
          satoshis: lndOpts.satoshis,
          tlscert: lndOpts['LND_TLS_CERT'],
      ...
      logger.warn(`Unable to connect and create payment channel with Core: ${coreIP} : ${error.message}`)
    }
...
async function submitHashAsync(hash) {
  ...
  for (let coreIP of env.CHAINPOINT_CORE_CONNECT_IP_LIST) {
    try {
      ...
      let invoiceAmount = decodedPaymentRequest.tokens
