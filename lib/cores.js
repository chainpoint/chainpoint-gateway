...
async function getCoreIPListFromSeedIPsAsync() {
  ...
  for (let seedIP of coreSeedIPs) {
    try {
      let coreResponse = (await coreRequestAsync(getPeersOptions, seedIP, 0)).body
...
async function getConnectedCoreIPsAsync(coreIPList, coreConnectionCount) {
  ...
  for (let coreIP of coreIPList) {
    try {
      let coreResponse = (await coreRequestAsync(getStatusOptions, coreIP, 0)).body
...
async function submitHashAsync(hash) {
  ...
  for (let coreIP of env.CHAINPOINT_CORE_CONNECT_IP_LIST) {
    try {
      ...
      responses.push({ ip: coreIP, response: submitResponse.body })
    } catch (error) {
...
async function getProofsAsync(coreIP, proofIds) {
  ...
  try {
    let coreResponse = (await coreRequestAsync(getProofsOptions, coreIP, 0)).body
    return coreResponse
  } catch (error) {
...
async function getCachedTransactionAsync(txID) {
  ...
  for (let coreIP of env.CHAINPOINT_CORE_CONNECT_IP_LIST) {
    try {
      let transaction = (await coreRequestAsync(getTxOptions, coreIP)).body
      if (transaction) {
