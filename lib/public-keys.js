...
async function getLocalPublicKeysAsync () {
  console.log('await rocksDB.getAllPublicKeysAsync()')
...
async function storeConfigPubKeyAsync (publicKeys) {
  console.log('storeConfigPubKeyAsync')
  let pubKeyItems = []
  for (var publicKeyHash in publicKeys) {
    ...
    if (publicKeys.hasOwnProperty(publicKeyHash)) {
      ...
      console.log(publicKeyHash, 'added')
    }
  }
  console.log('savePublicKeysAsync(pubKeyItems)')
