  ...
  if (pubKeys.length > 0) {
    ...
    for (let x = 0; x < pubKeys.length; x++) {
      ...
      if (pubKeys[x].pubKeyHash === pubKeyHash) {
        ...
        console.error(
          `ERROR : An invalid key ${pubKeys[x].pubKeyHash}:${
            pubKeys[x].pubKey
          } was found in the database and will be ignored`
        )
      }
    }
    return results
  ...
  for (var publicKeyHash in publicKeys) {
    if (publicKeys.hasOwnProperty(publicKeyHash)) {
      pubKeyItems.push({
        ...
        pubKey: publicKeys[publicKeyHash]
      })
    }
  }
...
async function getLatestCorePublicKeySetAsync(coreHosts, publicKeySet) {
  ...
  for (var publicKeyHash in coreConfig.public_keys) {
    if (
      coreConfig.public_keys.hasOwnProperty(publicKeyHash) &&
      !publicKeySet[publicKeyHash]
    ) {
      newKeys.push({
        pubKeyHash: publicKeyHash,
        pubKey: coreConfig.public_keys[publicKeyHash]
      })
    }
  }
