...
async function getCachedCoreProofsAsync(coreSubmissions) {
  ...
  for (let submitId in coreSubmissionsLookup) {
    coreSubmissionsLookup[submitId].proof = (function() {
      if (_.isNil(CORE_PROOF_CACHE[submitId].coreProof)) return null

      return CORE_PROOF_CACHE[submitId] ? CORE_PROOF_CACHE[submitId].coreProof : undefined
    })()
  }
  ...
  for (let index = 0; index < maxCoreSubmissionCount; index++) {
    ...
    for (let submissionsGroup of submissionsGroupByIPs) {
      ...
      try {
        logger.info(
          `getProofsFromCoreResults : ${JSON.stringify(flattenedSubmission)} : ${JSON.stringify(CORE_PROOF_CACHE)}`
        )
