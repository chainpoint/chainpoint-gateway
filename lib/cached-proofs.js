...
async function getCachedCoreProofsAsync(hashIdCores) {
  ...
  try {
    let cacheResults = hashIdCores.map(
      hashIdCore =>
        CORE_PROOF_DATA[hashIdCore]
          ? CORE_PROOF_DATA[hashIdCore].coreProof.toString('base64')
          : null
    )
    ...
    console.error(
      `Cache read error : getCachedCoreProofsAsync : ${error.message}`
    )
  }

  // for those not found in cache, retrieve from Proof Proxy
  let nullProofHashIdCores = coreProofs
    .filter(item => item.proof === null)
    .map(item => item.hash_id)
  let proofProxyResults = []
  if (nullProofHashIdCores.length > 0)
    proofProxyResults = await getProofsFromProofProxyAsync(nullProofHashIdCores)
  ...
  let finalProofResults = finalResultArray.map(coreProofItem => {
    if (coreProofItem.proof === null)
      return { hash_id: coreProofItem.hash_id, proof: null }
  ...
  let proofProxyResultIds = proofProxyResults.map(
    proofProxyResult => proofProxyResult.hash_id
  )
  let proofTypeLookup = finalProofResults.reduce((result, finalProofResult) => {
    if (
      proofProxyResultIds.includes(finalProofResult.hash_id) &&
      finalProofResult.proof
    ) {
      result[
        finalProofResult.hash_id
      ] = finalProofResult.anchorsComplete.includes('btc') ? 'btc' : 'cal'
      return result
    }
    return result
  }, {})
  ...
  let coreProofObjects = proofProxyResults.reduce((result, item, index) => {
    if (item.proof) {
      result.push({
        ...
        expiresAt:
          Date.now() +
          (proofTypeLookup[item.hash_id] === 'btc' ? 25 * 60 : 15) * 60 * 1000
      })
    }
    return result
  }, [])
  ...
  try {
    ...
    console.error(
      `Cache write error : saveCoreProofsToCacheAsync : ${error.message}`
    )
  }
...
async function getProofsFromProofProxyAsync(hashIdCores) {
  ...
  try {
    ...
    if (error.statusCode)
      throw new Error(`Invalid response on GET proof : ${error.statusCode}`)
