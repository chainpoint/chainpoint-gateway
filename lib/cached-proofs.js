...
async function getCachedCoreProofsAsync(coreSubmissions) {
  ...
  let [submitIdForHashIdCoreLookup, coreSubmissionsLookup] = coreSubmissions.reduce(
    (result, item) => {
      for (let core of item.cores) {
        result[0][core.hashIdCore] = item.submitId
      }
  ...
  for (let submitId in coreSubmissionsLookup) {
    coreSubmissionsLookup[submitId].proof = CORE_PROOF_CACHE[submitId]
      ? CORE_PROOF_CACHE[submitId].coreProof
      : undefined
  }
  ...
  for (let index = 0; index < maxCoreSubmissionCount; index++) {
    ...
    for (let submissionsGroup of submissionsGroupByIPs) {
      ...
      try {
        ...
        logger.error(
          `getCachedCoreProofsAsync : Core ${flattenedSubmission.ip} : HashIdCore Count ${
            flattenedSubmission.hashIdCores.length
          } (${JSON.stringify(flattenedSubmission.hashIdCores)}) : ${err.message}`
        )
      }
  ...
  for (let submitId in coreSubmissionsLookup) {
    coreSubmissionsLookup[submitId].anchorsComplete = utils.parseAnchorsComplete(
      coreSubmissionsLookup[submitId].proof,
      env.NETWORK
    )
  }
    ...
    let uncachedCoreProofObjects = newProofSubmitIds.reduce((result, submitId) => {
      if (!_.isNil(coreSubmissionsLookup[submitId].proof)) {
        result.push({
          ...
          expiresAt: Date.now() + (['btc', 'tbtc'].includes(proofTypeLookup[submitId]) ? 25 * 60 : 15) * 60 * 1000
        })
      }
      return result
    }, [])
