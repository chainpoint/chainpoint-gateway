...
async function initCoreHostsFromDNSAsync() {
  if (env.CHAINPOINT_CORE_API_BASE_URI === 'http://0.0.0.0') {
    ...
    if (hosts.length === 0)
      throw new Error('No Core instance DNS enteries located')
    ...
    try {
      await rocksDB.setAsync('CoreHostTXT', JSON.stringify({
        hosts: hosts
      }))
    } catch (error) {
...
async function coreRequestAsync(
  options,
  coreUriOverride,
  suppressRetryLogging
) {
  let coreUri = coreUriOverride ?
    `https://${coreUriOverride}` :
    await getCurrentCoreUriAsync()
  ...
  let miscOptions = (options.misc) ? JSON.parse(JSON.stringify(options.misc)) : {
    retries: 3
  }
  delete options.misc
  ...
  await retry(
    async bail => {
      ...
      retries: miscOptions.retries, // The maximum amount of times to retry the operation. Default is 10
      factor: 1, // The exponential factor to use. Default is 2
      minTimeout: 200, // The number of milliseconds before starting the first retry. Default is 1000
      maxTimeout: 400,
      randomize: true,
      onRetry: error => {
        if (!suppressRetryLogging)
          console.log(
            `INFO : Core request : ${error.statusCode ||
