...
async function coreRequestAsync (options) {
  ...
  await retry(async bail => {
    try {
      ...
      recentError = error
      if (error.statusCode) {
        // if there is a status code >= 500, then we should retry the call, throw an error
        if (error.statusCode >= 500) throw recentError
      } else {
        // No response was received, we should retry the call, throw an error
        throw recentError
      }
  ...
  // The retry loop will always store the most recent error received
  // If the retry routine succeeds at any interation, recentError will be set to null
  // If all retries fail, recentError will equal the last error received, throw that error
  // If there is a stausCode >= 500 or == null, the error will be thrown from inside the retry loop
  // If there is a statusCode < 500, we did not retry, and we handle that case below
  // This error is thrown because consumers of the coreRequestAsync function expect a response.body
  // to be returned, or an error thrown which they then handle
