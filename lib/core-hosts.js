...
async function initCoreHostsFromDNSAsync () {
  if (env.CHAINPOINT_CORE_API_BASE_URI === 'http://0.0.0.0') {
    ...
    let currentCoreHost
    try {
      let hostsString = await rocksDB.getAsync('CoreHostTXT')
      let hostsObject = JSON.parse(hostsString)
      currentCoreHost = hostsObject.hosts[Math.floor(Math.random() * hostsObject.hosts.length)]
    } catch (error) {
      throw new Error(`Could not get random CoreHostTXT item from Rocks`)
    }
