...
async function validateBlocksAsync (lastBlockRead, blocks) {
  ...
  let isValidSig = await publicKeys.validateSignatureAsync(lastBlockRead.hash, publicKeySet[pubKeyHash], sig)
...
async function validateNodeCalendarBlocksAsync (showLogging, lastBlockRead) {
  ...
  while (moreBlocksToRetrieve) {
    try {
      ...
      let blocks = await rocksDB.getCalendarBlockRangeAsync(startIndex, endIndex)
      if (blocks.length > 0) {
        try {
          ...
          console.log(`ERROR : Calendar : Could not validate blocks ${startIndex} to ${endIndex}`)
        }
      } else {
  ...
  while (checkNextRangeSet) {
    ...
    if (blockRangeTasks.length > 0) {
      try {
        ...
        console.error(`WARN : Calendar : Could not retrieve block range ${nextRangeNeededIndex} (blocks ${fromHeight} to ${toHeight})${error.statusCode ? ' : error ' + error.statusCode : ''}`)
        rangeError = true
  ...
  _.forEach(blocks, (block) => {
    if (block.type === 'reward') {
      ...
      if (env.NODE_TNT_ADDRESS === rewardAddress) {
        console.log(`INFO : Reward : ${rewardTNT} TNT received in transaction ${rewardTxId}`)
      }
    }
  })
}
...
async function startValidateRecentNodeAsync (ms) {
  try {
    ...
    let prevBlock = await rocksDB.getCalendarBlockByHeightAsync(startBlockHeight)
...
async function startCalculateChallengeSolutionAsync (ms, isPrivateNode) {
  ...
  if (coreConfig && !isPrivateNode) {
    try {
      ...
      await rocksDB.setAsync('challenge_response', `${challengeCreateTime}:${nodeChallengeResponse}`)
      ...
      let meetsMinVerRequirement = semver.satisfies(version, `>=${coreMinVersion}`)
      if (!meetsMinVerRequirement) console.log(`WARN : Version : This Node is running version ${version}, but the required minimum version for reward eligibility is ${coreMinVersion}`)
    } catch (error) {
