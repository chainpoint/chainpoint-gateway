...
async function validateSignatureAsync (blockHashHex, prefixedSig) {
  ...
  try {
    ...
    if (!publicKeySet[pubKeyHash]) {
      ...
      let newKeys = []
      for (var publicKeyHash in stackConfig.public_keys) {
        if (stackConfig.public_keys.hasOwnProperty(publicKeyHash) && !publicKeySet[publicKeyHash]) {
          newKeys.push({ pubKeyHash: publicKeyHash, pubKey: stackConfig.public_keys[publicKeyHash] })
        }
      }
...
async function getCalendarBlocksAsync (coreUri, startIndex, endIndex) {
  let options = {
    headers: {
      'Content-Type': 'application/json'
    },
    method: 'GET',
    uri: `${coreUri}/calendar/${startIndex}/${endIndex}`,
    json: true,
    gzip: true,
...
async function retrieveAndValidateNewGlobalBlocksAsync (lastBlockRead, stackConfig) {
  ...
  while (moreBlocksToRetrieve) {
    // for each batch of local calendar blocks received, validate block hashes for each block
    let response = await getCalendarBlocksAsync(coreUri, startIndex, endIndex)
...
async function startCalculateChallengeSolutionAsync (ms) {
  ...
  try {
    ...
    let coreChallenge = response.body.calendar.audit_challenge
...
async function syncCalendarAsync (isFirstRun, stackConfig, pubKeys) {
