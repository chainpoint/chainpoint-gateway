...
// TODO: this is the public key for verifying block signatures
// A key discovery mechanism and support for multiple possible
// determined by the sig prefix will be added
// prod: bOzyk1WTJHQCBu2SeFWn9wSlutdMV0f2SvSux9FpFPE=
// stag: Q88brO55SfkY5S0Rbnyh3gh1s6izAj9v4BSWVF1dce0=
const corePubKeyBytes = nacl.util.decodeBase64('Q88brO55SfkY5S0Rbnyh3gh1s6izAj9v4BSWVF1dce0=')
...
function validateSignature (blockHashHex, prefixedSig) {
  let blockHashBytes = nacl.util.decodeUTF8(blockHashHex)
  let signatureBytes = nacl.util.decodeBase64(prefixedSig.split(':')[1])
  let isValidSig = nacl.sign.detached.verify(blockHashBytes, signatureBytes, corePubKeyBytes)
  return isValidSig
}
...
function validateBlocks (lastBlockRead, blocks) {
  ...
  if (!lastBlockRead) { // the first block must be the genesis block
    ...
    let isValidSig = validateSignature(genBlockHashHex, blocks[0].sig)
  ...
  for (let x = 0; x < blocks.length; x++) {
    ...
    let isValidSig = validateSignature(blockHashHex, blocks[x].sig)
...
async function retrieveAndValidateNewGlobalBlocksAsync (lastBlockRead, stackConfig) {
  ...
  let baseURI = env.CHAINPOINT_CORE_API_BASE_URI
  let startIndex = lastBlockRead ? lastBlockRead.id + 1 : 0
...
async function startAuditLocalFullAsync (ms) {
  try {
    await validateLocalCalendarBlocksAsync(false)
...
async function startAuditLocalRecentAsync (ms) {
  try {
    let mostRecentBlockInfo = getMostRecentLocalBlockInfoAsync(false)
