...
async function getCoreCalendarBlockRangeAsync (blockRangeIndex) {
  ...
  try {
    ...
    if (error.statusCode) {
      ...
      throw new Error(`Invalid response : ${error.statusCode} : ${error.message}`)
    }
...
async function validateNodeCalendarBlocksAsync (showLogging, lastBlockRead) {
  ...
  while (moreBlocksToRetrieve) {
    ...
    if (blocks.length > 0) {
      ...
      if (showLogging) console.log(`Validated Node blocks ${startIndex} to ${lastBlockRead.id}`)
...
async function retrieveAndValidateNewCoreBlocksAsync (lastBlockRead, coreConfig) {
  ...
  while (checkNextRange) {
    ...
    try {
      ...
      console.error(`Could not read calendar block range ${nextRangeNeededIndex}, retrying in 5 seconds`)
      setTimeout(function () {
    ...
    if (blocks.length === 0) {
      ...
      console.log(`Blockrange ${nextRangeNeededIndex} not ready yet`)
      checkNextRange = false
      return
    }
    // the blockrange was found, process it and continue
    ...
    console.log(`Validated and stored Core blocks ${nextRangeNeededIndex * BLOCKRANGE_SIZE} to ${lastBlockRead.id}`)
...
async function startValidateFullNodeAsync (ms) {
  try {
    ...
    console.error(`Error performing full calendar validation : ${error.message}`)
  } finally {
...
async function startValidateRecentNodeAsync (ms) {
  try {
    ...
    console.error(`Error performing recent calendar validation : ${error.message}`)
  } finally {
