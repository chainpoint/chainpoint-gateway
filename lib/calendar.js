...
async function retrieveAndValidateNewCoreBlocksAsync (coreConfig, rangeSetSize, isPeriodicUpdate) {
        ...
        if (blocks.length > 0) {
          ...
          setImmediate(async () => {
            try {
              ...
              console.log(`INFO : Calendar : Validated ${startId} to ${endId} (New)`)
              writeOpCount--
            } catch (error) {
              console.log(`ERROR : Calendar : Could not write blocks ${startId} to ${endId}`)
              process.exit(1)
            }
          })
        }
  ...
  let pendingWrites = writeOpCount
  while (pendingWrites > 0) {
    await utils.sleepAsync(100)
    pendingWrites = writeOpCount
  }
}
...
async function syncNodeCalendarAsync (isFirstRun, coreConfig, pubKeys) {
  ...
  await retrieveAndValidateNewCoreBlocksAsync(coreConfig, 10, false)
}
