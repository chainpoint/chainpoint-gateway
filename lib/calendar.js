...
async function validateBlocksAsync (lastBlockRead, blocks, verifySig) {
  ...
  if (!lastBlockRead) { // the first block must be the genesis block
    ...
    if (verifySig) {
      ...
      if (!isValidSig) throw new Error(`ERROR : Invalid signature block at height ${blocks[0].id}`)
    }
  ...
  for (let x = 0; x < blocks.length; x++) {
    ...
    if (verifySig) {
      ...
      if (!isValidSig) throw new Error(`ERROR : Invalid signature block at height ${blocks[x].id}`)
    }
...
async function retrieveAndValidateNewCoreBlocksAsync (lastBlockRead, coreConfig) {
  ...
  while (checkNextRange) {
    ...
    try {
      ...
      console.error(`ERROR : Could not read calendar block range ${nextRangeNeededIndex}${error.statusCode ? ': error ' + error.statusCode : ''}`)
      /*
      setTimeout(function () {
