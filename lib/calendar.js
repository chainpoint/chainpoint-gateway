...
async function getMostRecentNodeBlockInfoAsync (showLogging) {
  ...
  if (!mostRecentBlockInfo) {
    if (showLogging) console.log(`INFO : Calendar : Initializing...`)
    return null
  }
  ...
  while (checkNextRange) {
    ...
    if (!rangeError) {
      ...
      if (blocks.length === 0) {
        ...
        console.log(`INFO : Calendar : Validated ${nextRangeNeededIndex * BLOCKRANGE_SIZE} to ${lastBlockRead.id} (New)`)
...
async function startCalculateChallengeSolutionAsync (ms) {
  try {
    ...
    try {
      ...
      if (error.statusCode) throw new Error(`Calendar : Could not retrieve Core config : ${error.statusCode}`)
      throw error
    }
