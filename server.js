...
async function startAsync () {
  try {
    ...
    console.log(`await migrateHMACKeysAsync()`)
    await migrateHMACKeysAsync()
    // Register HMAC Key
    console.log(`await authKeysUpdate()`)
    await authKeysUpdate()
    console.log(`let hmacKey = await registerNodeAsync(nodeUri)`)
    let hmacKey = await registerNodeAsync(nodeUri)
    console.log(`apiServer.setHmacKey(hmacKey)`)
    apiServer.setHmacKey(hmacKey)
    console.log(`let coreConfig = await coreHosts.getCoreConfigAsync()`)
    let coreConfig = await coreHosts.getCoreConfigAsync()
    console.log(`let pubKeys = await initPublicKeysAsync(coreConfig)`)
    let pubKeys = await initPublicKeysAsync(coreConfig)
    console.log(`await eventMetrics.loadMetricsAsync()`)
    await eventMetrics.loadMetricsAsync()
    console.log(`await migrateCalendarDataAsync()`)
